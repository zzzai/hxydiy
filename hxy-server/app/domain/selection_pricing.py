...
def calculate_selection_pricing(
    ...
    for index, item in enumerate(items):
        ...
        if option_charge is not None:
            ...
            if project:
                try:
                    project_price_snapshot = price_book_snapshot(db, project.id)
                    prices = project_price_snapshot.prices
                except ValueError:
        ...
        if project is not None:
            ...
            source_ref = {
                "price_book_project_id": project.id,
                "price_book_id_by_type": (
                    project_price_snapshot.id_by_type if project_price_snapshot is not None else {}
                ),
                "price_book_version_by_type": (
                    project_price_snapshot.version_by_type if project_price_snapshot is not None else {}
                ),
                "price_book_source_type_by_price_key": (
                    project_price_snapshot.source_type_by_price_key
                    if project_price_snapshot is not None else {}
                ),
                "confirmed_price_source_type": source_type,
            }
            if project_price_snapshot is not None and source_type in project_price_snapshot.id_by_type:
                source_ref["confirmed_price_book_id"] = project_price_snapshot.id_by_type[source_type]
                source_ref["confirmed_price_book_version"] = project_price_snapshot.version_by_type[source_type]
            pricing_line["price_source_ref"] = source_ref
        if option_charge is not None:
    ...
    return {
        ...
        # 给顾客端和前台统一展示“门店价可省多少会员价”的依据；
        # 该字段只比较两种价格带，不包含非会员券，避免营销券与会员权益重复计算。
        "member_saving_cents": max(
            0,
            max(0, store_subtotal + adjustment_store)
            - max(0, member_subtotal + adjustment_member),
        ),
