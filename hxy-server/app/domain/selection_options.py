...
def _linked_project_item(
    ...
    return {
        ...
        "quantity": 1,
        "addon_ids": [],
        "diy_preferences": [body_part] if body_part else [],
        "item_type": "service",
        "chargeable": True,
...
def split_choice_snapshots(
    ...
    for choice in selected:
        ...
        if linked.category == "local-strength":
            body_part = "".join(unicodedata.normalize("NFKC", choice.name or "").split())
            if body_part in _LOCAL_BODY_PARTS:
                snapshot["body_part"] = body_part
...
def merge_linked_service_units(
    ...
    for linked in linked_items:
        ...
        if linked.get("catalog_reference_only"):
            linked_key = _service_unit_key(linked)
            if linked_key is not None and linked_key[1] is not None:
                target = by_key.get(linked_key)
