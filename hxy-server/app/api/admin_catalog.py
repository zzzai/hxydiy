...
from app.api.admin_v2 import _audit, _is_headquarters_admin, _require_catalog_master_admin, _staff_store_id
...
def _group_for_project(db: Session, project_id: int, group_id: int, staff: Staff) -> tuple[Project, ProjectCatalogVersion, ProjectOptionGroup]:
    ...
    if version is None or project is None or project.id != project_id or (not _is_headquarters_admin(staff) and project.store_id != _staff_store_id(staff)):
        raise HTTPException(status_code=404, detail="选项组不存在")
...
def create_option_group(
    ...
    db.add(group)
    _flush_or_conflict(db)
    _audit(db, staff, "create_option_group", "project_option_group", str(group.id), {"project_id": project.id, "catalog_version_id": version.id})
    _commit_or_conflict(db)
    db.refresh(group)
...
def patch_option_group(
    ...
    _audit(db, staff, "update_option_group", "project_option_group", str(group.id), {"project_id": project.id, "catalog_version_id": version.id})
    _commit_or_conflict(db)
    db.refresh(group)
    return {**_group_out(db, group), "catalog_version": _version_meta(version)}
...
def delete_option_group(
    ...
    db.delete(group)
    _audit(db, staff, "delete_option_group", "project_option_group", str(group.id), {"project_id": project.id, "catalog_version_id": version.id})
    _commit_or_conflict(db)
    return {"ok": True}
...
def create_option_choice(
    ...
    _audit(db, staff, "create_option_choice", "project_option_choice", str(choice.id), {"project_id": project.id, "catalog_version_id": version.id, "group_id": group.id})
    _commit_or_conflict(db)
    db.refresh(choice)
...
def patch_option_choice(
    ...
    _audit(db, staff, "update_option_choice", "project_option_choice", str(choice.id), {"project_id": project.id, "catalog_version_id": version.id, "group_id": group.id})
    _commit_or_conflict(db)
    db.refresh(choice)
    return {**_choice_out(db, choice), "catalog_version": _version_meta(version)}
...
def delete_option_choice(
    ...
    db.delete(choice)
    _audit(db, staff, "delete_option_choice", "project_option_choice", str(choice.id), {"project_id": project.id, "catalog_version_id": version.id, "group_id": group.id})
    _commit_or_conflict(db)
    return {"ok": True}
...
def publish_project_catalog(
    ...
    _audit(db, staff, "publish_catalog_version", "project", str(project.id), {"catalog_version_id": published.id, "version": published.version})
    _commit_or_conflict(db)
    return {
        "id": published.id,
        "project_id": project.id,
