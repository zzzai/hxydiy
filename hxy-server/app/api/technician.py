...
def current_technician(authorization: str | None, db: Session) -> tuple[Staff, Technician]:
    ...
    if (
        not technician
        or technician.store_id != staff.store_id
        or technician.status not in {"available", "busy"}
    ):
        raise HTTPException(
            status_code=401,
            detail={
                "code": "TECHNICIAN_ACCOUNT_UNAVAILABLE",
                "message": "技师当前不在岗或账号不可用",
            },
        )
    ...
    for room in rooms:
        ...
        if len(room_occupancies) > 1:
            items.append({
                ...
                "items": [],
                "conflict": True,
                "conflict_count": len(room_occupancies),
            })
            continue
...
def _technician_occupancy(db: Session, occupancy_id: int, technician: Technician) -> PositionOccupancy:
    occupancy = db.scalar(
        ...
        .where(PositionOccupancy.id == occupancy_id)
        .with_for_update()
    )
...
def _reject_conflicted_room_action(db: Session, occupancy: PositionOccupancy) -> None:
    """Do not let a direct API request bypass the board's room-conflict guard."""
    ...
    if len(active_ids) > 1:
        raise HTTPException(
            status_code=409,
            detail={
                "code": "POSITION_OCCUPANCY_CONFLICT",
                "message": "该房间存在多个活动服务记录，请联系店长核对现场服务位",
            },
        )
...
def _action(occupancy_id: int, action: str, body: ActionIn, authorization: str | None, db: Session) -> dict:
    ...
    replay = db.scalar(
        ...
        .where(
            StateTransition.store_id == technician.store_id,
    ...
    result = {
        ...
        "service_status": occupancy.status,
        "resource_status": room.status if room else None,
        "resource_control": "external_read_only",
    ...
    try:
        ...
        # 并发重试可能在本次事务提交前写入相同幂等键；回滚后读取首笔结果。
        db.rollback()
        replay = db.scalar(
        ...
        raise HTTPException(
            status_code=409,
            detail={"code": "IDEMPOTENCY_KEY_REUSED", "message": "该幂等键已用于其他服务操作，请生成新的幂等键"},
        )
    return result
