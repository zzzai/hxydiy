...
def _require_manager(staff) -> None:
    if normalize_staff_role(staff.role, staff.technician_id) != "manager":
        raise HTTPException(
            status_code=403,
            detail={"code": "MANAGER_REQUIRED", "message": "技师账号仅可查看服务单"},
        )
...
def live_board(
    ...
    active_occupancies = {
        occupancy.active_room_id: occupancy
        for occupancy in db.scalars(
            select(PositionOccupancy).where(
                PositionOccupancy.store_id == store_id,
                PositionOccupancy.active_room_id.in_([item.id for item in rooms]),
                PositionOccupancy.status != "released",
            )
        )
        if occupancy.active_room_id is not None
    }
    ...
    return {
        ...
        "resources": {
            ...
            "rooms": [{
                "id": item.id,
                "name": item.name,
                "status": active_occupancies[item.id].status if item.id in active_occupancies else item.status,
...
def counter_checkout_selection(
    ...
    order = Order(
        ...
        member_discount_cents=max(
            0,
            int(pricing_snapshot.get("store_total_cents", payable_total) or payable_total)
            - int(pricing_snapshot.get("member_total_cents", payable_total) or payable_total),
        ),
...
def settle(
    ...
    order.member_discount_cents = max(
        0,
        int(settlement_pricing.get("store_total_cents", final_payable_total) or final_payable_total)
        - int(settlement_pricing.get("member_total_cents", final_payable_total) or final_payable_total),
    )
    if automatic_coupon.coupon_id is not None:
