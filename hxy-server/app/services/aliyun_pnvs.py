...
def _send_request(phone: str):
    try:
        ...
        dypnsapi_models = SimpleNamespace(
            SendSmsVerifyCodeRequest=lambda **kwargs: SimpleNamespace(**kwargs)
        )
        util_models = SimpleNamespace(RuntimeOptions=lambda: SimpleNamespace())
...
def verify_sms_code(phone: str, code: str) -> bool:
    ...
    try:
        ...
        from types import SimpleNamespace
        dypnsapi_models = SimpleNamespace(
            CheckSmsVerifyCodeRequest=lambda **kwargs: SimpleNamespace(**kwargs)
        )
        util_models = SimpleNamespace(RuntimeOptions=lambda: SimpleNamespace())
