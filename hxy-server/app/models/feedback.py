...
class VisitFeedback(Base):
    ...
    __table_args__ = (
        UniqueConstraint("identity_hash", "idempotency_key_hash", name="uq_visit_feedback_identity_key"),
        Index("ix_visit_feedback_rate_scope", "identity_hash", "service_position_qr_id", "created_at"),
    )

    id: Mapped[int] = mapped_column(primary_key=True)
    store_id: Mapped[int] = mapped_column(ForeignKey("stores.id"), index=True)
    room_id: Mapped[int] = mapped_column(ForeignKey("rooms.id"), index=True)
    service_position_qr_id: Mapped[int | None] = mapped_column(
