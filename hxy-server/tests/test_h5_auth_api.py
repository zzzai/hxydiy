...
class H5AuthApiTests(unittest.TestCase):
    ...
    def test_phone_login_merges_anonymous_completed_record_without_persisting_phone_identity_in_browser_cookie(self):
        ...
        with self.SessionLocal() as db:
            ...
            self.assertEqual(visit.user_id, user.id)
    ...
    def test_h5_me_returns_latest_membership_for_existing_token(self):
        with self.SessionLocal() as db:
            user = User(openid="h5_membership_refresh", phone="13300133000", is_member=False)
            db.add(user)
            db.commit()
            db.refresh(user)
        ...
        self.assertEqual(response.json()["member_type"], "annual")

    def test_h5_me_requires_a_valid_bearer_token(self):
