...
class AdminCatalogOptionsApiTests(unittest.TestCase):
    def setUp(self):
        ...
        with self.SessionLocal() as db:
            ...
            db.flush()
            self.hq_admin = Staff(username="catalog-hq-admin", name="总部目录管理员", role="admin", status="active", password_hash=hash_password("pass"), store_id=None)
            ...
            self.staff = Staff(username="catalog-staff", name="目录店长只读账号", role="manager", status="active", password_hash=hash_password("pass"), store_id=self.store.id)
            ...
            db.add_all([self.hq_admin, self.admin, self.staff, self.other_admin])
            db.flush()
            ...
            self.hq_admin_id = self.hq_admin.id
        ...
        # 目录主数据写入必须使用未绑定门店的总部管理员；绑定门店的 admin/manager 仅代表店长。
        self.admin_headers = {"Authorization": f"Bearer {create_staff_token(self.hq_admin_id, 'admin')}"}
        self.staff_headers = {"Authorization": f"Bearer {create_staff_token(self.staff_id, 'manager')}"}
    ...
    def test_admin_can_publish_project_with_free_linked_and_custom_options(self):
        ...
        with self.SessionLocal() as db:
            ...
            self.assertEqual(published.published_by, self.hq_admin_id)
    ...
    def test_project_crud_is_strict_and_protects_published_link_references(self):
        ...
        self.assertEqual(create.json()["code"], "STRICT-CREATE")
