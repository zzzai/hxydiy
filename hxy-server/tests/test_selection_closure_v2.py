...
class SelectionClosureV2Tests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with cls.SessionLocal() as db:
            ...
            headquarters = Staff(
                username="closure-v2-headquarters",
                password_hash=hash_password("pass"),
                name="总部管理员",
                role="admin",
                store_id=None,
            )
            db.add_all([staff, headquarters])
            db.commit()
            cls.store_id = store.id
            cls.project_id = project.id
            cls.staff_id = staff.id
            cls.headquarters_staff_id = headquarters.id
    ...
    def test_member_login_hint_uses_current_payable_after_promotion(self):
        from app.api.selections import _saving_hint

        hint = _saving_hint(self.SessionLocal(), self.store_id, {
            "store_total_cents": 28600,
            "member_total_cents": 19600,
            "payable_total_cents": 27600,
        }, None)

        self.assertEqual(hint, {"kind": "member", "estimated_saving_cents": 8000, "login_required": True})

    def test_logged_in_customer_can_bind_current_anonymous_selection(self):
        session_id, token = self.create_session()
        with self.SessionLocal() as db:
            anonymous = User(openid="anon_bind_current_selection")
            customer = User(
                openid="h5_bind_current_selection",
                phone="13800138018",
                is_member=True,
                member_type="annual",
            )
            db.add_all([anonymous, customer])
            db.flush()
            db.get(SelectionSession, session_id).customer_id = anonymous.id
            db.commit()
            customer_id = customer.id

        response = self.client.post(
            f"/api/v1/selection-sessions/{session_id}/bind-customer",
            headers={
                "X-Selection-Token": token,
                "Authorization": f"Bearer {create_access_token(str(customer_id), 'h5_bind_current_selection')}",
            },
        )
        ...
        with self.SessionLocal() as db:
            self.assertEqual(db.get(SelectionSession, session_id).customer_id, customer_id)
        ...
        self.assertIsNone(quote.json()["saving_hint"])
    ...
    def test_admin_can_publish_priced_and_free_addons_for_customer_catalog(self):
        headers = {"Authorization": f"Bearer {create_staff_token(self.headquarters_staff_id, 'admin')}"}
