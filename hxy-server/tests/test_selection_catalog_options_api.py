...
def expanded_scenario(scenario: CatalogSelectionScenario) -> CatalogSelectionScenario:
    with _session_local()() as db:
        ...
        qiqing_local_leg_choice = ProjectOptionChoice(
            option_group_id=qiqing_local_group.id,
            code="local-leg",
            name="腿部",
            choice_type="linked_project",
        ...
        db.add_all([qiqing_local_choice, qiqing_local_leg_choice])
        ...
        scenario.qiqing_local_leg_choice_id = qiqing_local_leg_choice.id
    return scenario
...
def test_local_catalog_choices_expose_parts_and_keep_two_service_units(
    expanded_scenario: CatalogSelectionScenario,
):
    catalog = expanded_scenario.client.get(
        f"/api/v1/projects?store_id={expanded_scenario.store_id}"
    )

    assert catalog.status_code == 200, catalog.text
    qiqing = next(
        item for item in catalog.json()["items"]
        if item["id"] == expanded_scenario.qiqing_id
    )
    local_group = next(group for group in qiqing["option_groups"] if group["code"] == "local")
    assert [choice["body_part"] for choice in local_group["choices"]] == ["肩颈", "腿部"]
    ...
    response = _post_revision(expanded_scenario, [
        {
            ...
            "catalog_version_id": expanded_scenario.qiqing_version_id,
            "option_choice_ids": [
                expanded_scenario.qiqing_local_choice_id,
                expanded_scenario.qiqing_local_leg_choice_id,
            ],
        },
    ...
    assert response.status_code == 200, response.text
    items = _items(response)
    parent = next(item for item in items if item["project_id"] == expanded_scenario.qiqing_id)
    assert [
        snapshot["body_part"]
        for snapshot in parent["catalog_selection"]["linked_snapshots"]
    ] == ["肩颈", "腿部"]
    local_items = [
    ...
    assert [item["source_option_choice_ids"] for item in local_items] == [
        [expanded_scenario.qiqing_local_choice_id],
        [expanded_scenario.qiqing_local_leg_choice_id],
    ]
