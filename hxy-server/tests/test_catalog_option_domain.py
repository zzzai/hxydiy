...
class CatalogOptionDomainTests(unittest.TestCase):
    ...
    def test_validate_catalog_allows_distinct_local_body_parts_on_one_linked_project(self):
        with self.SessionLocal() as db:
            store = self._add_store(db)
            root = self._add_project(db, store, "local-parts-root")
            linked = self._add_project(db, store, "local-parts-linked")
            linked.category = "local-strength"
            db.add(PriceBook(project_id=linked.id, price_type="store", amount_cents=2000))
            self._add_version(db, linked, status="published")
            version = self._add_version(db, root)
            group = self._add_group(db, version, "local-strength", max_select=5)
            for display_order, (code, name) in enumerate(
                (("local-shoulder-neck", "肩颈"), ("local-leg", "腿部"))
            ):
                choice = self._add_choice(
                    db,
                    group,
            ...
            self.assertNotIn("duplicate_linked_project", {error.code for error in errors})

            duplicate = self._add_choice(
                db,
                group,
                "local-shoulder-neck-copy",
                choice_type="linked_project",
                linked_project_id=linked.id,
                charge_mode="inherit_linked_price",
                display_order=2,
            ...
            self.assertIn("duplicate_linked_project", {error.code for error in errors})
