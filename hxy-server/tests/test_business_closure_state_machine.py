...
class BusinessClosureApiContractTests(unittest.TestCase):
    def setUp(self):
        ...
        with self.SessionLocal() as db:
            ...
            db.add_all([project, room_container, technician, second_technician])
            db.flush()
            room.parent_room_id = room_container.id
            second_room.parent_room_id = room_container.id
            db.add_all([room, second_room])
            db.flush()
            order = Order(
            ...
            self.room_container_id = room_container.id
            self.room_id = room.id
            self.second_room_id = second_room.id
            self.technician_id = technician.id
            self.second_technician_id = second_technician.id
    ...
    def assign(
        ...
        *,
        ...
        return self.post(
            ...
            {
                "idempotency_key": key,
                "technician_id": technician_id or self.technician_id,
    ...
    def test_two_beds_in_the_same_room_can_run_in_parallel(self):
        first_visit_id = self.check_in(self.order_id, "check-in-bed-a").json()["visit_id"]
        second_visit_id = self.check_in(self.second_order_id, "check-in-bed-b").json()["visit_id"]

        first = self.assign(first_visit_id, "assign-bed-a")
        second = self.assign(
            second_visit_id,
            "assign-bed-b",
            technician_id=self.second_technician_id,
            room_id=self.second_room_id,
        )

        self.assertEqual(first.status_code, 200, first.text)
        self.assertEqual(second.status_code, 200, second.text)
        with self.SessionLocal() as db:
            container = db.get(Room, self.room_container_id)
            first_bed = db.get(Room, self.room_id)
            second_bed = db.get(Room, self.second_room_id)
            self.assertEqual(container.status, "available")
            self.assertEqual(first_bed.status, "reserved")
            self.assertEqual(second_bed.status, "reserved")
            self.assertEqual(db.scalar(select(func.count(ServiceAssignment.id))), 2)
    ...
    def test_live_board_resource_status_reflects_active_diy_occupancy(self):
        with self.SessionLocal() as db:
            session = SelectionSession(
                id="closure-live-board-diy",
                access_token_hash="live-board-diy-token",
                store_id=self.store_id,
                status="submitted",
            )
            db.add(session)
        ...
        response = self.client.get("/api/v1/operations/live-board", headers=self.headers)

        self.assertEqual(response.status_code, 200, response.text)
        room = next(item for item in response.json()["resources"]["rooms"] if item["id"] == self.room_id)
        self.assertEqual(room["status"], "waiting_service")
