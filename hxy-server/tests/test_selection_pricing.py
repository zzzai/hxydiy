...
class SelectionPricingTests(unittest.TestCase):
    ...
    def test_pricing_exposes_member_saving_for_non_member_comparison(self):
        with self.SessionLocal() as db:
            pricing = calculate_selection_pricing(
                db,
                [{"project_id": self.standard_id, "quantity": 1, "state": "confirmed"}],
                price_type="store",
            )

        self.assertEqual(pricing["store_total_cents"], 7990)
        self.assertEqual(pricing["member_total_cents"], 5990)
        self.assertEqual(pricing["member_saving_cents"], 2000)

    def test_project_pricing_line_freezes_price_book_ids_and_versions(self):
        with self.SessionLocal() as db:
            rows = list(db.query(PriceBook).filter_by(project_id=self.standard_id))
            for row in rows:
                row.version = f"menu-20260825-{row.price_type}"
            db.commit()
            expected_ids = {row.price_type: row.id for row in rows}
            expected_versions = {row.price_type: row.version for row in rows}
        ...
        source = pricing["lines"][0]["price_source_ref"]
        self.assertEqual(source["price_book_project_id"], self.standard_id)
        self.assertEqual(source["price_book_id_by_type"], expected_ids)
        self.assertEqual(source["price_book_version_by_type"], expected_versions)
        self.assertEqual(source["confirmed_price_source_type"], "member")
