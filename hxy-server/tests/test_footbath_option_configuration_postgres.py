...
def _seed_configuration(SessionLocal, suffix: str, *, include_low_scope_project: bool = False):
    with SessionLocal.begin() as setup:
        ...
        targets = [
            _add_project(setup, store.id, code, category_by_code[code])
            for code in TARGET_CODES
        ]
        small_projects = [
            _add_project(setup, store.id, code, "small")
            for code in SMALL_OPTION_CODES
        ]
...
def test_apply_locks_complete_project_scope_in_one_order_before_revalidation():
    ...
    try:
        ...
        assert f"missing published or priced option projects: {SMALL_OPTION_CODES[0]}" in str(error)
...
def test_concurrent_applies_serialize_without_duplicate_drafts():
    ...
    try:
        ...
        assert sorted(report.created_groups for report in reports) == [0, 17]
        assert sorted(report.created_choices for report in reports) == [0, 68]
        with SessionLocal() as verification:
            assert verification.scalar(
                select(func.count()).select_from(ProjectCatalogVersion)
            ) == 5
            assert verification.scalar(select(func.count()).select_from(ProjectOptionGroup)) == 17
            assert verification.scalar(select(func.count()).select_from(ProjectOptionChoice)) == 68
        print(f"DUAL_APPLY_SERIALIZED pids={pids} versions=5 groups=17 choices=68")
    finally:
        for thread in threads:
            if thread.is_alive():
