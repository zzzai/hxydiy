...
from app.api.admin_v2 import reject_selection_change_request
...
class SelectionAdminApiTests(unittest.TestCase):
    ...
    def test_rejecting_service_addition_uses_the_same_lock_order_as_approval(self):
        source = getsource(reject_selection_change_request)

        self.assertIn("_locked_owned_selection", source)
    ...
    def test_selection_list_exposes_bound_customer_identity(self):
        ...
        self.assertEqual(record["customer"]["phone"], "136****6000")
        self.assertNotIn("13600136000", response.text)
