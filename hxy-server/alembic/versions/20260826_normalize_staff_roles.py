...
def normalize_roles(bind) -> None:
    ...
    Bound legacy ``staff`` rows are technician identities; unbound ``staff``
    rows are retained as the formal read-only store employee role.
    """
    bind.execute(sa.text("UPDATE staff SET role = 'manager' WHERE role = 'admin'"))
    bind.execute(sa.text("UPDATE staff SET role = 'technician' WHERE role = 'staff' AND technician_id IS NOT NULL"))
