...
from app.services.aliyun_pnvs import AliyunPnvsError, send_sms_code, verify_sms_code
...
def send_h5_code(body: H5SendCodeRequest, db: Session = Depends(get_db)) -> H5SendCodeResponse:
    ...
    if settings.environment == "production" and not settings.h5_sms_debug and not settings.aliyun_pnvs_enabled:
