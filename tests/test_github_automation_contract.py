...
class GitHubAutomationContractTests(unittest.TestCase):
    ...
    def test_trusted_gate_runs_pr_code_without_secrets_and_writes_a_head_check(self):
        ...
        self.assertIn("github.rest.checks.create", content)
        self.assertNotIn("github.rest.checks.update", content)
        self.assertIn("name: Trusted job / Static contracts", content)
        self.assertIn("'Trusted / Static contracts': process.env.STATIC_RESULT", content)
        self.assertIn("name: Trusted job / Admin tests and build", content)
        self.assertIn("name: Trusted job / Customer tests and build", content)
        self.assertIn("name: Trusted job / Backend tests", content)
