...
type HistoryItem = {
  ...
  profile_summary: TechnicianHistoryProfileSummary | null;
};
...
export default function TechnicianServiceHistoryPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setUnassignedLegacyCount(response.data?.unassigned_legacy_count || 0);
    } catch {
      setFailed(true);
    } finally {
      setLoading(false);
  ...
  return <section className="technician-own-history">
    ...
    {failed ? <Alert type="error" showIcon message="本人历史加载失败" description="请检查网络后重试。" action={<Button size="small" onClick={() => void load()}>重试</Button>} />
      : loading && !items.length ? <div className="technician-loading"><Spin /></div>
        : !items.length ? (() => {
          const emptyState = technicianHistoryEmptyState(status, unassignedLegacyCount);
          if (emptyState === 'legacy') return <div className="technician-empty-state"><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="旧数据未关联" /><Typography.Paragraph type="secondary">有 {unassignedLegacyCount} 条本店已结束旧服务无法确认服务技师，不会猜测归入本人历史。</Typography.Paragraph></div>;
          if (emptyState === 'filtered') return <div className="technician-empty-state"><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={status === 'confirmed' ? '当前筛选下尚无顾客已确认记录' : '当前筛选下尚无待补记服务'} /></div>;
          return <div className="technician-empty-state"><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="尚无本人已完成服务" /></div>;
        })() : <>
          <div className="technician-history-list">{items.map((item) => <article className="technician-history-card" key={item.occupancy_id}>
            <div className="technician-history-card-main">
              ...
              <Typography.Paragraph className="technician-history-summary">{technicianHistorySummaryLines(item.profile_summary).join(' · ') || '本次尚未形成顾客确认的长期摘要'}</Typography.Paragraph>
            </div>
          </article>)}</div>
