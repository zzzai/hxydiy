...
- `CUSTOMER-FEEDBACK-005`：在 `codex/feedback-recovery` 收拢反馈后端、签名入口校验和顾客端入口，状态仍为 `Developing`（本地集成，不代表发布）。证据与未关闭门禁见 `docs/operations/feedback-recovery-20260923.md`。
- 本批次不叠加 `wb-multi-store` 的模板/价格扩展；保留该分支成果，待评价链路与迁移应用历史核验后继续。
- 下一门禁：候选 HEAD CI/独立审查、PostgreSQL 并发、迁移演练、正式签名门店码及三端联调。未获许可不执行生产数据库升级。
