...
- 每个会员最多一个 `active` 会员核验入口；浏览器 Cookie 仅是入口凭证，不代表物理手机。清除浏览器数据、无痕模式或更换浏览器均视为新入口。
...
- `POST /auth/h5/trusted-device/rebind`：当前有效登录会话提交本人手机号短信验证码后，原子撤销旧入口和未消费码，启用当前浏览器入口；不改变会员身份或直接应用会员价。
...
- 顾客可通过当前手机号的短信验证码自助切换核验入口；频繁换绑、跨店异常或其他风险由店长处理换绑与审计。管理后台不承担日常扫码，不提供手机号直接套会员价或前端手改价格。
