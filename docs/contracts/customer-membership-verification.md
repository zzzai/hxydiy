...
- `GET /technician/membership-verification/selections`：只返回当前门店待服务/服务中的、仍持有活动服务位关联的最小选单摘要；同一服务位存在多条活动关联时，该服务位不在 `items` 中，而以 `blocked_positions` 返回名称和活动记录数。服务结束后为下一单保留的历史记录不参与候选。
