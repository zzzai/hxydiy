...
v6 历史记录按服务单项目快照编码 `hxy-xiaoqi-90` 识别；只有旧快照没有编码时才使用精确名称“招牌草本泡”。不修改项目名称、价格、时长或实际执行状态。新的服务结束记录统一使用 v5 三步服务连续性流程：服务方式、实际调整、反馈与下次；仅显示与项目相关的字段。已存在 v6 仍可由本人查看和更正，不迁移、不覆盖。v1–v5 历史同样不删除或迁移。
...
v6 允许 `correction_of_id` 指向同门店、同顾客、同服务单、同技师、同创建账号的 v6 记录，并要求原因；锁定原记录检查，已有更正则返回 409。原记录不可覆盖，更正不增加服务次数。v1–v4 更正权限不扩展；v5 更正规则见 `service-reference-v4.md`。
