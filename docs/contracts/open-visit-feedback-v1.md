...
- 每个门店的每个实际服务位各有一张签名二维码；不是每店共用一张码。匿名与登录顾客扫同一张床位码，都能先直接评价，也能选择进入原有选项目流程。
...
- 顾客端扫描床位码后先提供“评价与建议”和“选项目”两个动作；评价及令牌过期续期只调用独立到店入口，不因床位占用而失败。选择项目才沿用 `POST /api/v1/entry-sessions`，继续遵守现有占位规则。
