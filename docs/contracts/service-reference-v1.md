...
- `schema_version=4`、`taxonomy_version=service_reference_v3` 在原有快记字段外增加 `customer_reported.body_service_notes`，最多 3 条。
- 每条仅含稳定编码 `area`、`context` 与 `reconfirm_next_visit`；表达的是顾客自述的服务相关情况，不是技师诊断。
- 可选部位为肩颈、背部、腰臀、手臂、膝周、腿部、腹部、足部、皮肤；可选情况为旧伤、术后恢复、近期不适、长期不适、皮肤敏感、下次确认。
- 身体条目不进入 `customer_profile_current`、自动分群、定价或营销；历史仅显示“服务前再确认”。
