...
- `GET /api/v1/admin/v2/service-reference-summary` 仅供总部管理员或当前门店店长读取，服务端按登录账号门店范围统计。
- 响应仅包含 `total`、`confirmed`、`confirmation_rate_percent`、`corrected` 和 `correction_rate_percent` 五个数值。不得返回顾客、技师、原话、备注、服务方式、身体部位或其他可识别/自由文本字段。
- 统计范围为 `schema_version` 为 3、4、5 的当前有效记录；已被后续更正记录替代的原记录排除，更正记录本身仍计入总数及更正数。确认率和更正率均以 `total` 为分母；无有效记录时返回 `0.0`。
