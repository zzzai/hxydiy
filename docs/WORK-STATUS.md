# 2026-09-24 1 号店「辅」目录生产发布

- PR #160 精确 HEAD `39074258ae5cc4d48c3c90b670a8136e34779bbb` 合并为主干 `4da67f05fa12fe58861f0d540a5266a7750f9463`；主干 CI `35968638396` 成功。生产部署 `35971527614` 的准入、备份、隔离恢复/迁移、生产迁移、部署和健康检查作业成功。服务器 current 为 `github-4da67f05fa12-35971527614`，Alembic revision 为 `20260924_aux_visibility`，公网健康接口返回 production / ok。
- 发布前备份 `pre-github-4da67f05fa12-35971527614-20260924T075012Z.dump` 为 602708 字节，SHA-256 校验通过。生产目录脚本先只读预览 10 项预期变更，再执行 `python -m scripts.reconcile_aux_menu --store-id 1 --apply`；二次只读预览返回空变更。直接执行文件路径会因容器模块搜索路径缺少 `/app` 报 `ModuleNotFoundError`，该次未写入数据；后续统一使用 `python -m`。
- 公开项目 API 实测六项顺序与门店/会员价为头疗 79/49、足部精修 69/39、精油开背 99/69、采耳 89/59、局部推拿 79/49、拔罐/刮痧 59/29。旧拔罐 ID 8、刮痧 ID 9 仍可读取详情；主项目 ID 1 的已发布目录仍引用两者。新合并项目的目录为必选单选，两项选择均免费。足部精修旧门店价 5900 分的版本已结束，新版本为 6900 分；旧价格行未改写。生产订单数仍为 0、选单数仍为 533、旧项目被引用的选项行仍为 30。没有制造测试订单或改写历史选单。
- 顾客端真实浏览器/微信真机、门店现场及历史“腰臀”文案的独立版本化修订仍待完成；本记录不等于现场验收。

# 2026-09-24 有效签名服务位码取消浏览器旧位置阻断

- PR #161 以精确 HEAD `44808b34b3b1cc62dd1b771868b065469f72f421` 通过门禁并合并为主干 `988f65d7e8cd4e6bd3ac717e56bfa360e0046afc`；主干 CI `35966142007` 成功。
- 生产部署 `35966439083` 的资格校验、备份、恢复演练、部署和健康检查成功；服务器 current 为 `github-988f65d7e8cd-35966439083`，公网 `/api/v1/health` 返回 production / ok，发布包后端包含有效签名码优先于浏览器旧位置门禁的入口逻辑。
- 有效签名码进入目标服务位时，目标已有活动占用则加入共享选单，目标可用则创建独立草稿；原位置及其选单不自动释放、不覆盖、不跨位带入。停码、错码、无签名入口、暂停/清洁/维修状态和限流未放宽，无数据库迁移。
- 本地专项占用/共享入口 60 项、后端全量 594 项（3 项跳过）及仓库静态合同 34 项通过。尚未使用微信真机重新扫描现场 A4 短链码，本条不代表门店现场验收完成。

# 2026-09-24 顾客菜单独立茶饮选购板块移除

- PR #158 合并为主干 `c3f45002471c579df3fa2580928feea1b7d43c48`；主干 CI `35960479710` 成功，生产部署 `35960696936` 的资格校验、备份、恢复演练、部署和健康检查成功。
- 服务器 current 为 `github-c3f45002471c-35960696936`；公网 `/api/v1/health` 返回 production / ok，首页引用顾客端 JS `/assets/index-B1cZrhaI.js`。
- 生产浏览器以门店 1、2 号沙发入口核验：分类导航从“泡脚沐足”开始，不存在“茶 / 茶饮”分类、“茶｜茶饮”分区或“五行茶饮”选购卡片；泡脚、推拿和 SPA 项目说明中的“茶饮”文字按边界保留。
- 本次未删除茶商品、门店供应配置或历史服务单数据，也未修改 API、数据库、价格、项目目录和会员规则。用户微信真机刷新复验尚未完成，不能用自动化浏览器替代现场确认。

# 2026-09-24 “座位有人”继续进入菜单生产发布

- PR #156 合并为主干 `ebc21372ab81a952c1807380ceb0a784f7517f38`；主干 CI `35956097544` 成功，生产部署 `35956358269` 的资格校验、备份、恢复演练、部署和健康检查成功。
- 服务器 current 为 `github-ebc21372ab81-35956358269`；公网 `/api/v1/health` 返回 production / ok，发布包 OpenAPI 与线上顾客端 JS `/assets/index-CjFV-FSm.js` 均包含新增的 `entry_notice`。
- 有效签名二维码现在允许进入“门店状态有人、但无活动 DIY 选单”的目标位置，显示一次轻提示后进入菜单并建立共享草稿；空闲位置双占、无签名入口、清洁/维修/暂停、跨位置数据、版本冲突和提交锁定不变。无数据库迁移。
- 未为验证而修改生产座位状态；微信真机扫码该特定场景仍待门店现场验收，不能以 CI、发布成功或 HTTP 200 替代。

# 2026-09-23 服务位短链接生产发布

- 业务 PR #153 提供短链实现；独立迁移许可 PR #154 合并为主干 `922a19c6eaaa852635f3537a06843b29183ae2fc`。主干 CI `35878238079` 成功，生产部署 `35878718292` 的备份校验、隔离恢复升级、生产迁移、部署和健康检查成功。
- 服务器 current 为 `github-922a19c6eaaa-35878718292`，生产 Alembic head 为 `20260923_qr_short_code`；公网健康接口 200，OpenAPI 包含 `/api/v1/q/{short_code}`，错误格式返回 403。
- 使用现有授权服务位二维码记录完成一次管理端短链读取及公网 307 验证，跳转目标为既有签名 H5。该记录的 ID、随机标识、服务位绑定、状态和替换关系保持不变，active 记录总数保持不变；只按设计惰性补写短码哈希。未轮换、停用或改绑现有 8 条二维码记录，也未输出短码明文。
- 8 张二维码图片的导出、打印、逐位核对和微信真机现场验收尚未执行，不以本次生产验证替代。

# 2026-09-23 已占用服务位二维码继续进入菜单

- PR #151 合并为主干 `21391f87f730941afb7d1348214f13238f8a74c4`；主干 CI `35854344816` 成功，生产部署 `35856025262` 成功。
- 有效签名二维码指向已有活动占用时，即使当前浏览器绑定过其他服务位，也只提示一次并直接进入目标共享草稿或只读菜单；原服务位不释放、不覆盖。目标为空时仍返回 `BROWSER_ACTIVE_ELSEWHERE`，旧式无签名入口也不放宽。
- 本地顾客端 226 项通过、1 项跳过，生产构建通过；后端共享专项 10 项及占用相关合计 57 项通过。公网健康接口、入口和新 JS `/assets/index-BMR6ebJK.js` 均 HTTP 200，新提示已存在。微信真机扫描已占用座位及多人增删仍待现场验收，不能以部署成功替代。

# 2026-09-23 顾客端共享选单常驻提示移除

- PR #149 合并为主干 `ad068bff6bb927e6b0c0c8b1523568988eb5f2ef`；主干 CI `35852482084` 成功，生产部署 `35852809475` 的资格校验与“Backup, rehearse, deploy and verify”作业成功。
- 仅移除普通共享草稿菜单顶部的常驻说明条；共享选单、多人自动同步、版本冲突和提交锁定逻辑均未修改。清单已提交后的只读提醒继续保留。
- 顾客端本地 225 项通过、1 项跳过，生产构建通过。公网首页、健康接口及新 JS `/assets/index-CVegN5S-.js` 核验通过；被删除文案已不存在，只读提醒仍存在。真机缓存刷新与门店视觉复核未执行。

# 2026-09-23 顾客端服务位多人共享选单生产发布与浏览器验收

- 业务 PR #138 合并为 `f80b633843dbc12eb5e0340ce552c435c725de53`；首轮主干 CI `35842678289` 的失败仅来自两份设计文档尾部空行，其他业务作业因静态合同先失败而未运行。PR #139 修正文档格式后，主干 `77b4c818605fc0533e56d76ccaef27ce5d0abf15` 的 CI `35842988929` 全量成功。包含本功能的后续主干 `314f7eec241f2dd0f59914e586d163007b1f846b` 的 CI `35843262747` 与生产部署 `35843520748` 均成功；发布作业实际执行了备份、恢复演练、部署与健康校验。
- 公网 `/api/v1/health` 返回 production / ok，`/api/v1/openapi.json` 含 `X-Collaboration-Token`，线上顾客 JS 含共享草稿、只读浏览和 409 冲突处理。两台隔离浏览器使用 1 号沙发有效签名码加入同一未提交草稿，获得不同协作者令牌；共同增选可自动同步，陈旧写入返回 409 且保留最新项目，菜单首屏及匿名「评价与建议」入口正常。
- 为避免污染真实门店，本轮没有提交测试服务单；测试结束前已清空草稿项目，未使用管理后台强制释放空占用。提交幂等/原子锁定、跨床隔离和本人服务评价权限由后端合同与集成测试覆盖，但尚未在生产制造订单验证。微信真机、弱网、多人并发、停码/轮换、已完成服务本人评价和营业现场仍待验收，不能以浏览器自动化或部署成功替代。
