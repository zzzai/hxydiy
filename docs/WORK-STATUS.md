...
# 2026-08-30 技师区位看板孤立床位修复（本地完成，未发布）

## 本地完成

- 修复技师今日服务看板查询：未归属房间的 `bed` 记录不再作为独立服务位展示。
- 保持当前业务规则：看板展示 8 个沙发和 7 个房间，房间不拆分床位；不涉及派单或智慧宝物理资源管理。
...
- 技师后端相关筛选专项：`38 passed, 1 warning`。
- 管理端测试：`107 passed`。
- 管理端生产构建：成功；保留既有共享 chunk 体积警告。

## 发布状态

- 本轮未发布生产，未执行数据库备份、迁移、Manifest 发布切换或 API 重建。
- 服务器实际 `current` 未改变，仍为 `customer-profile-member-no-coupon-20260829-1`。

## 尚未完成事项

- 待门店授权手机完成“顾客提交 → 技师查看区位和选单 → 确认服务 → 服务结束 → 顾客画像快记保存/重试 → 审计核对”闭环。
- 自动化测试不替代真实手机、断网、并发和智慧宝并行操作验收。
