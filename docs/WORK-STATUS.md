# 2026-09-08 技师服务交接卡生产发布

- 服务交接卡功能 PR #55 的业务提交为 `b23f66736631741e64a85ee1ecd6945d2f709536`。后续 PR #56 只修正文档空白，纯文档 CI 不产生双端构建产物，发布按门禁正确跳过。
- PR #58 合并提交 `660ac718bc0b0e04153176ebc074f5784cce4384` 修复受控恢复发布路径：手工恢复必须指定当前 `main`、成功 CI 运行及双端构建产物，自动 `push` 发布门禁未放宽。专项契约测试 `12 passed`。
- 主干 CI `34229848361` 的静态、管理端、顾客端、后端和发布就绪检查全部成功；生产部署 `34230016610` 成功。服务器 `current` 为 `github-660ac718bc0b-34230016610`，已只读核验后端 `communication_preference` 与已发布前端 `recording_outcome`，公网健康接口 HTTP 200。
- 本次无数据库迁移。自动化发布的备份、恢复演练、Manifest、原子切换和健康检查均已通过；真实微信和门店现场验收未执行，不以此替代上线事实。
