...
- 生产 release：未变更。
# 2026-09-03 管理端普通员工只读角色收口（本地完成，未发布）

## 本地完成

- 修复后端认证层与前端权限契约不一致：门店绑定的 `staff` 账号可登录管理后台并保持只读角色。
- 未绑定门店的员工账号返回 `STORE_BINDING_REQUIRED`；技师仍要求绑定技师档案。
- 更新角色迁移脚本与专项测试，避免把普通员工误判为待迁移技师。

## 验证结果

- Python `compileall`：通过。
- 源码工作区既有后端专项回归：57 passed；当前任务 worktree 未安装 pytest，需在完整虚拟环境复跑。
- 管理端 npm 测试、TypeScript 检查、生产构建：待执行。

## 发布状态

- 未发布生产；未修改服务器、数据库或生产 `current`。

## 尚未完成事项

- 完成管理端全量验证和后端专项复跑后推送 PR。
- 生产发布仍需数据库备份、Manifest、原子切换、健康检查和普通员工现场权限验收。
