...
# 2026-08-30 技师服务动作与画像契约发布

## 本地完成

- 发布前后端技师/画像专项测试 `22 passed`；管理端测试 `110 passed`；`npx tsc -b` 与 `npm run build` 均通过（Vite 转换 `4001` 个模块）。
- 本次发布包含服务确认/结束幂等键作用域修复：幂等键绑定服务位、动作和登录技师，跨服务位、跨动作或跨技师复用返回 `409 IDEMPOTENCY_KEY_REUSED`。
- 顾客服务参考备注上限统一为 `500` 字；年龄段和安全标签统一，移除医疗化或经营性旧文案。

## 已发布生产

- Release：`technician-profile-contract-20260830-1`；服务器 `current` 已切换到 `/root/hxy-diy-20260811/releases/technician-profile-contract-20260830-1`。
- 已完成数据库备份 `/root/hxy-diy-20260811/backups/pre-technician-profile-contract-20260830-193734.dump` 和环境文件备份；本次未执行数据库迁移，Alembic head 保持 `20260830_media_assets`。
- 发布前、切换后均已验证 `MANIFEST.sha256`，当前文件 SHA-256 为 `e3e438dc53a6d930ccebf9ecd6bd5bec0a32d009b2b67836da0aa3103f443be1`；Compose 配置校验通过。
- 公网 `https://diy.hexiaoyue.com/api/v1/health` 返回 `200`；`hxy-diy-api` 容器状态为 `running`、重启次数为 `0`。
- 线上复核跨服务位重放同一幂等键返回 `409 IDEMPOTENCY_KEY_REUSED`，未产生写入；授权测试技师仅完成登录和只读看板核验，确认显示 `8` 个沙发、`7` 个房间及三栏底部导航，随后已退出登录。

## 未随本次发布内容

- 媒体上传、七牛配置及其他未单独验证的管理端改动未混入本 release。
- 未修改顾客端业务逻辑，未加入派单、接单、认领，未执行智慧宝开房、开沙发、派钟、离位、清洁或 DIY 物理资源释放。

## 待门店现场验收

- 使用真实门店账号完成顾客提交、技师看板同步、确认服务、服务结束、服务参考保存/失败重试与审计核对闭环。
- 验收弱网/断网恢复、并发重复点击、账号权限和门店隔离；自动化与线上只读复核不等同营业现场验收。

## 2026-08-30 技师端生产可发布项复核（无新增切换）

### 复核结果

- 当前技师端源码与生产 `current` 的 `technician.py`、`technician_admin.py` 内容一致；服务动作幂等键作用域、手机区位看板、账号生命周期和画像快记已包含在 `technician-profile-contract-20260830-1`。
- 本轮未发现新的技师端代码或数据库迁移可安全制作 release，因此不重复生成空 release、不切换 `current`。

### 验证结果

- 后端技师/画像专项：`22 passed`；管理端测试：`110 passed`；管理端类型检查和生产构建通过（Vite `4001` modules）。
- 生产数据库 Alembic：`20260830_media_assets (head)`；Manifest 全量校验通过，文件 SHA-256 为 `e3e438dc53a6d930ccebf9ecd6bd5bec0a32d009b2b67836da0aa3103f443be1`。
- 公网健康接口和 `/technician/` 入口均返回 `200`；未登录技师任务接口按预期返回 `401`；API 容器 `running`、重启次数 `0`。

### 发布状态

- 已发布生产：继续使用 `/root/hxy-diy-20260811/releases/technician-profile-contract-20260830-1`，本轮无新增发布。
- 媒体/七牛、顾客端及其他管理端未验证改动不在技师端发布范围内。
