# 2026-09-14 会员迁移与当前主干生产发布

- PR #88 已合并，主干提交 `eebd7c1e4066d1e3c6f69344c58aa83b7f281af4` 将经批准的 `20260910_membership_closure.py` 加入生产迁移白名单；该改动的 CI `34809438687` 成功。
- 运行时代码主干 `efb8c187fdb280f90cad159e81d03dda8cd22651`（PR #89，且包含技师服务交接 PR #84）可信 CI `34809757938` 成功，正式部署 `34810436752` 成功。
- 发布作业已完成服务器备份校验、恢复演练、受控 Alembic 迁移、不可变包校验、原子切换和三入口健康检查。发布后公网 `/api/v1/health`、`/admin/`、`/` 分别返回 HTTP 200。
- `c5473351e6c7154afe2f1d5c456a7f5e8f16b445` 仅补充管理端合并状态文档，无运行时代码差异，因此未重复发布。微信真机与门店营业现场验收尚未执行。
