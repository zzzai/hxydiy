# 2026-09-23 品牌总部菜单允许清单生产发布

- PR #146 合并为主干 `7125930c9c3d3fd78875498e1ae67954dc609604`；主干 CI `35850024744`、显式生产部署 `35850385797` 的资格校验及“Backup, rehearse, deploy and verify”作业成功。
- 总部只显示经接口契约确认支持总部范围的六个页面；技师、顾客、标签、分群、房间、优惠券、DIY 页面配置和 SCRM 等门店专属页在总部隐藏并由旧地址守卫阻止挂载。店长门店工作区菜单保持不变，后端门店隔离未放宽。
- 本地管理端 216 项测试与生产构建通过；发布后公网健康接口 production / ok，`/admin/` 和本次构建 JS `/admin/assets/index-C_gCqg0U.js` 均 HTTP 200。服务位二维码管理既有后端专项 57 项与前端专项 9 项测试通过；1 号店公开服务位地图返回 8 个服务位。这些都不替代真实账号进入两个工作区、逐位查看/停用/重新启用二维码、浏览器打印和门店现场扫码验收。

# 2026-09-23 品牌总部保留旧地址的路由守卫生产发布

- PR #144 合并为主干 `390eb2b2028bede0211ff709257df5e2f18152cb`，主干 CI `35847716336` 成功；显式生产部署 `35848074456` 的资格校验与“Backup, rehearse, deploy and verify”作业成功。
- 上次只改变默认落点；浏览器保留 `#/today` 时，总部仍可挂载门店页面并触发两个 403。本次按工作区是否绑定门店过滤门店运营导航和地址访问，总部旧地址会转回总部默认页，店长/门店工作区行为不变，服务端门店隔离未放宽。
- 本地管理端 215 项测试与构建通过；发布后公网 `/api/v1/health` 为 production / ok，`/admin/` 与本次构建 JS `/admin/assets/index-DpYwrnS8.js` 均 HTTP 200。真实浏览器旧地址跳转、总部菜单和门店工作区仍待人工验收。

# 2026-09-23 品牌总部工作区默认页面修复生产发布

- PR #142 合并为主干 `33f1eaf6c376b2bd9770aff6bb4850a1fa7772c0`；主干 CI `35845807723` 成功，显式生产部署 `35846236622` 的资格校验及“Backup, rehearse, deploy and verify”作业成功。
- 截图中的两次“当前账号未绑定门店”是总部登录后默认加载门店“今日运营”的统计与服务单接口，均按既有门店权限返回 403。总部账号不应被绑定到某一门店；本次只将其默认落点改为总部可用的“门店主数据”，保持店长落点及服务端权限不变。
- 本地管理端 214 项测试与生产构建通过。发布后公网 `/api/v1/health` 为 production / ok，`/admin/` 及本次构建的 `/admin/assets/index-U9-K8EWA.js` 均 HTTP 200。真实浏览器重新登录、总部门店列表及门店工作区切换仍待人工验收。

# 2026-09-23 管理后台工作区登录角色契约修复生产发布

- PR #140 已合并为主干 `314f7eec241f2dd0f59914e586d163007b1f846b`，主干 CI `35843262747` 成功；显式生产部署 `35843520748` 的资格校验及“Backup, rehearse, deploy and verify”作业均成功。
- 生产接口实测：账号拥有 `brand_admin` 与 `store_manager` 两个工作区；选择接口返回的 `staff.role` 分别为原始工作区角色，而旧前端误按 `admin` / `manager` 校验，导致截图中的“工作区选择结果不匹配”。本次前端在校验后转换会话角色，同时修复单工作区直登。账号密码重设及授权状态变更是独立操作，不在本 PR 内。
- 本地管理端 213 项测试与生产构建通过；发布后公网 `/api/v1/health` 为 production / ok，`/admin/` 和本次构建对应的 `/admin/assets/index-BlQvvRoo.js` 均 HTTP 200。真实浏览器重新登录总部/门店及菜单权限仍待人工验收，不能以公网探针代替。

# 2026-09-23 管理后台多工作区选择修复与授权恢复

- PR #136 合并为主干 `ff3f4675810b9910d1351e9a44413899a0fc9c35`；主干 CI `35838800537` 与手动生产部署 `35839162357` 成功。服务器 `current` 已指向 `github-ff3f4675810b-35839162357`，后台和健康接口 HTTP 200，线上脚本包含工作区选择文案。
- 生产 `admin` 的现有 1 号店 `store_manager` 授权 ID 4 在发布后从 disabled 恢复为 active，品牌管理员授权 ID 1 保持 active；凭证版本由 4 增至 5，系统审计 ID 1157 记录操作原因和前后状态。没有新建重复授权。
- 管理端本地 212 项测试与生产构建、后端工作区授权 16 项测试通过。真实账号在浏览器分别进入总部与门店、店长二维码管理、微信现场扫码仍待人工验收；公网 200 不代表这些场景已完成。

# 2026-09-23 到店反馈、品牌授权与顾客菜单同步生产发布

- PR #130、#132、#133 的业务实现和 PR #134 的独立迁移许可均已合并；精确发布主干为 `34fab31cbcb8d3a27801fe81bd6539a93233c241`。主干 CI `35833984523` 成功并生成双端构建产物，显式生产部署 `35834287369` 的资格校验与“Backup, rehearse, deploy and verify”作业均成功。
- 受保护发布先备份并校验 `pre-github-34fab31cbcb8-35834287369-20260923T080016Z.dump`，隔离恢复库先升级，再升级生产库；生产 Alembic 为 `20260921_staff_scope`，`visit_feedback` 与 `staff_scope_assignments` 均存在，`current` 指向 `github-34fab31cbcb8-35834287369`。公网顾客页、后台页和健康接口 HTTP 200，线上 JS 哈希匹配当前 release。
- 按用户确认，给生产 `admin` 增加 1 号店 `store_manager` 工作区，保留其 `brand_admin` 工作区；凭证版本递增以撤销旧会话，授权审计记录已核实。门店尚未张贴二维码；微信真机、真实床位扫码、匿名/登录评价、跨店隔离、弱网与重复提交的现场验收未完成。

# 2026-09-21 技师服务交接记录重构生产发布

- PR #128 已合并技师记录重构，PR #129 已修复文档静态门禁；最终主干提交为 `cfa957c512573330db60016eca7caf13898e6332`。
- 手动补跑的完整主干 CI `35590967031` 全部成功，并生成 `admin-dist-cfa957c512573330db60016eca7caf13898e6332` 与 `customer-dist-cfa957c512573330db60016eca7caf13898e6332`。生产部署 `35591215199` 成功，实际“Backup, rehearse, deploy and verify”作业已执行，未被跳过。
- 技师服务后记录现以沟通偏好、身体部位与下次动作、本次实际调整为核心；年龄段和性别按需补充，性别选项仅男/女，私人备注仅创建技师本人可见。历史 v1-v6 保持兼容，管理端只接收需要下次确认的抽象状态，不接收身体细节、年龄、性别或私人备注。
- 同批上线服务单中文展示及会员扫码先行流程：不再向技师展示 `#position-1` 等内部编码；会员核验无需预先选择服务位，扫码后再匹配候选服务单，最终核销仍由后端校验。
- 发布后独立公网核验：`/`、`/admin/`、`/technician/`、`/api/v1/health` 均返回 HTTP 200。未执行微信真机、弱网、重复提交或门店真实营业验收，状态为 `Deployed`，不等于 `Field Accepted`。

# 2026-09-21 顾客端沙发编号与平面图生产更新

- PR #125 合并为主干 `9b60d3a3b6f60ff2cef161e396d29c718cd3bd77`，可信主干 CI `35579488848` 成功。顾客端不再按数组奇偶拆分座位，而是按 `map_x` 分左右、按 `map_y` 排上下。
- 保留二维码及历史稳定编码 `sofa-01` 至 `sofa-08`，仅将显示号映射为 `1、2、3、5、6、7、8、9`。数据库事务更新 5 行；未改二维码、占用、状态机、排序、坐标或历史服务单关联。
- 当前完整 release 为 `github-c76ac301efa0-35576431361`，在该生产基线上热更新顾客端静态构建 `421212a`；操作前备份为 `/root/hxy-diy-20260811/backups/pre-sofa-renumber-20260921T084829Z.dump`。
- 发布后公网服务位地图、首页 bundle 与健康接口均通过：左侧坐标对应 `1、2、3、5`，右侧对应 `6、7、8、9`，首页引用 `/assets/index-CZ7oTNho.js`，健康状态 `ok`。微信真机视觉仍待现场验收。

# 2026-09-21 商品目录首期可运营闭环生产发布

- 业务实现 PR #121 已合并为 `7edae827787df5ff0d25350b6d08e9e2e6a70615`；迁移发布许可 PR #123 已合并为精确主干提交 `c76ac301efa07f803d251453a5c99151b4a62dc4`。主干 CI `35575952327` 成功，显式生产部署 `35576431361` 成功。
- 受保护发布先生成并校验备份 `pre-github-c76ac301efa0-35576431361-20260921T081716Z.dump`，再对恢复演练库和生产库执行 `20260910_membership_closure -> 20260921_product_catalog`。服务器 `current` 已切换至 `github-c76ac301efa0-35576431361`，上一 release 为 `manual-main-c3919ff`。
- 发布后独立公网核验：`/`、`/admin/`、`/technician/`、`/api/v1/health` 均返回 HTTP 200，健康状态为 production / ok。1 号店公开商品接口返回 6 条，新增字段齐全且排序契约通过。
- 状态为 `Deployed`，不等于 `Field Accepted`。尚需用总部管理员和绑定门店店长的授权测试账号，验收会员价设置/清空、展示顺序、详情模块、本店上下架和跨店隔离；未执行真实营业数据写入。

# 2026-09-18 顾客端茶饮与品牌配图生产发布

- PR #113 合并为 `c3919ff`，上线菊花清润茶、薏米祛湿茶、桑葚滋养茶及三张荷小悦 IP 品牌杯 WebP；PR #114 合并为 `285a233`，修复接口 `/diy/assets/` 地址在顾客端的生产静态路径映射。
- 完整发布目录为 `/root/hxy-diy-20260811/releases/manual-main-c3919ff`，部署前备份为 `/root/hxy-diy-20260811/backups/pre-manual-main-c3919ff-20260918T102454Z.dump`；路径修复随后仅热更新顾客端静态构建，未重建 API 或操作数据库。
- 发布后只读核验：首页引用 `/assets/index-BKhw3lzL.js`，页面内容接口返回三款新茶饮，三张图片均 HTTP 200，健康接口返回 `ok`，API 容器运行且重启次数为 0。微信真机视觉与缓存刷新仍待现场验收。

# 2026-09-17 技师看板服务单计数澄清生产发布

- PR #110 已合并为主干提交 `f6ab1f98c91cdef4981885aaf7627a5b03566ab9`。可信 CI `35199237392` 成功；维护者显式触发的生产部署 `35201013678` 成功完成，其中“Backup, rehearse, deploy and verify”作业已通过。
- 已发布：今日看板不再把“服务完成·仍在位”与待服务/服务中的订单混写为“当前 N 个服务单”；首屏分别呈现服务中、待服务、待核对和已完成数量，列表标题只显示总数。该改动不改变服务状态、接口载荷、权限、门店隔离、幂等或审计。
- 发布后只读公网核验：`/api/v1/health` 返回 production / ok，`/technician/` 返回 HTTP 200。未执行数据库迁移、价格调整、服务位状态机或智慧宝物理资源操作。
- 自动部署和公网探针不替代微信真机、弱网、重复点击及门店真实营业现场验收；本次尚未单独在真机上复验新的数量文案。

# 2026-09-16 技师服务连续性收敛生产发布

# 2026-09-16 技师服务连续性生产浏览器验收

- 在生产“模拟技师-P0验收”测试门店，以匿名测试选单和授权测试技师账号完成两轮浏览器验收。第一轮服务单从顾客提交后在技师看板显示为待确认；服务结束后，“本次无补充”成功保存并出现在本人历史。
- 对第一轮记录完成一次带原因更正，只填写并确认温度适中、希望安静、本次合适、下次延续本次。本人历史仅展示这四项可行动摘要；未填写身体情况或私人笔记。第二轮从同一浏览器提交的匿名选单不继承该参考，验证系统没有把浏览器/设备当作自然人身份。
- 两张测试服务单均已完成，测试技师已退出。此为生产浏览器验收，不等同于微信真机、摄像头、弱网/重复提交或真实营业现场验收；这些场景及跨角色可见范围仍待执行。

- 主干提交 `3484080a900947c59456a4c272d1bc7ff0317fd6`（PR #107）已由可信 CI `35081036887` 验证并通过显式生产部署 `35081465247`；“Backup, rehearse, deploy and verify”作业成功完成。
- 已发布：上次服务安全摘要展示实际调整；技师可从本人 v5 历史回填并追加一次带原因更正，原记录保留。服务端继续限定同门店、同顾客、同次已完成服务、同创建账号，保留角色、服务状态、幂等和审计校验。管理端与其他技师不接收私人文字或身体细节。
- 本地验证：管理/技师端 `npm test` 198 通过、生产构建通过；后端技师专项 38 通过（1 条既有 TestClient 弃用警告）。发布后独立公网核验 `/api/v1/health`、`/admin/`、`/` 均为 HTTP 200。
- 未执行数据库迁移、价格变更、服务位状态机或智慧宝物理资源操作。待门店使用授权测试账号完成微信真机、服务前查看、服务后记录/更正、弱网重试和跨角色可见范围验收；自动化发布不替代现场验收。

# 2026-09-14 顾客端五方卡片生产发布

- PR #95 合并 SHA `2f9566e3e8e2ae85412cf70a1d5d1cd76b8d1732`；主干 CI `34820540269`、显式触发的部署 `34823261580` 及实际发布作业成功。服务器 current `github-2f9566e3e8e2-34823261580`，Manifest 校验通过，Docker API 与数据库正常，健康接口 ok。
- 五方卡片、独立药材图、OFL 圆润字体、重点多按一会/再放松一会排序上线；CSS `index-Cxa_lsTg.css`、JS `index-BVTuRAKA.js`、五方图片及字体 HTTP 200，生产浏览器切换图文正常。
- 目录 ID 9 / version 3 发布：复用经业务字段一致性校验的草稿，仅替换免费药方选项，保留其他组和价格。先事务回滚预演再提交，系统审计 actor `customer-herbal-release`，旧目录 ID 4 快照验证通过。未执行数据库结构迁移；现场验收未进行。
- 本地顾客端 192 passed / 1 skipped，生产构建通过；药方专项 15 passed；PR/主干 CI 通过。网络不稳时通过受信任服务器内存管道访问 GitHub API，凭证未写入文件或日志；没有跳过门禁。

# 2026-09-14 管理端当前阶段能力生产发布

- 手动发布任务 `34810436752` 成功部署主干提交 `efb8c187fdb280f90cad159e81d03dda8cd22651`（PR #89 的合并提交，包含此前已合并的年度会员闭环、媒体引用保护、项目销量与门店员工账号管理）。
- 服务器 `current` 已切换至 `/root/hxy-diy-20260811/releases/github-efb8c187fdb2-34810436752`；公网 `/`、`/admin/`、`/technician/` 与 `/api/v1/health` 均为 HTTP 200。
- 这是维护者手动触发的 `workflow_dispatch`，不是主干合并自动发布。自动化发布与公网探针不替代人工验收：仍需在真实门店验证年度会员开通/续费/退款、短信自助换机、赠送服务行核销、员工账号调岗/停用和媒体解除引用后的删除。

# 2026-09-14 管理端媒体删除引用保护已合并，未发布

- PR #85 已 squash 合并，主干提交：`20fafaaa8d70f5dda22a1ad058ebd2facb8f89ce`；可信 CI `34803424268` 成功。
- 本轮没有执行数据库迁移、服务器代码切换、容器重建或健康检查。合并后被自动触发的生产部署工作流 `34803530982` 已取消，未将 `current` 切换到该提交；当前生产事实仍以 `CURRENT-STATE.md` 和服务器实时核验为准。
- 待人工现场验收：总部/店长跨店隔离、引用中媒体删除被拒绝、解除引用后的删除与审计记录。

# 2026-09-10 顾客评价与项目分享生产发布

- PR #67 已 squash 合并，主干提交：`d7ca1f78341b1108746a2d0f62bb29297b4a3eb2`。
- GitHub 可信 CI `34366546055` 和生产部署 `34366753130` 均成功；服务器 `current` 只读核验为 `/root/hxy-diy-20260811/releases/github-d7ca1f78341b-34366753130`。
- 已发布：评价不预设五星、按评分展示分层标签且最多三项、300 字选填说明与隐私提示；评价成功后可返回本人到店记录或再次选购。低分自动回访、运营跟进和技师考核未实现。
- 已发布：项目详情安全分享入口和公开 Open Graph 落地页。线上 `hxy-xiaoqi-90` 分享页、其缩略图、顾客首页和健康接口均 HTTP 200；分享 URL 不含服务位、二维码、选单、浏览器令牌、登录令牌或手机号。
- 本次没有数据库迁移、价格规则或服务位状态机变更。自动化与公网探针不替代真机微信分享、聊天卡片缩略图、弱网评价重试和门店营业验收。

# 2026-09-09 服务笔记重设计发布

- PR #66 合并提交 `cae3f208c68c37247e1d9f477f58970290bfe6d8`；CI `34359213054`、部署 `34359434662` 均 success，后台报告已终结为 deployment_succeeded。
- 常用字段直接呈现、笔记不再以结果选择为前提，身体情况同页填写。前端 178 项测试、构建和本地模拟接口浏览器验证通过；无迁移。
- 公网新版 JS/CSS 与健康接口已核验。服务器 current 未重新读取，微信与门店验收未完成，不以自动化代替现场事实。

# 2026-09-09 技师极简服务后快记生产发布

- PR #63 已合并，主干提交：`85c8f46410bb4705b1c60332772d934d9846a356`。
- GitHub 可信 CI `34304218505` 与生产部署 `34304347004` 均成功，发布报告为 `deployment_succeeded`；本次没有数据库迁移。
- 默认页只呈现沟通方式、关键调整、结果与可选交接一句话；温度、避让和下次提醒保持折叠。身体情况改为独立按需入口，先选大部位再进入精确点位，复用既有 v5 载荷与三项上限。
- 发布后只读公网核验：`/technician/`、`/api/v1/health` 均 HTTP 200，入口脚本包含“交接一句话”“先选常用部位”。这不替代微信真机、弱网和门店服务闭环验收。

# 2026-09-07 技师已完成服务单参考入口修复发布

- PR #51 已 squash 合并，主干提交：`f2f893797011aea1f9aa924edf2686c5b18af8f8`。
- GitHub 可信 CI `34126497735` 与生产部署 `34126720373` 均成功，发布报告为 `deployment_succeeded`；本次仅改技师端前端，不执行数据库迁移。
- 修复已完成服务单仍展示“查看上次服务参考”而调用活动服务位专用接口返回 409 的问题：入口现在只在“确认服务”或“服务结束”等活动服务状态显示，服务端状态、门店与隐私校验未改变。
- 授权测试技师账号已在生产页面复验已完成服务单：仅显示“填写服务参考”，无效入口已消失。真实微信摄像头、弱网和门店营业流程仍须按现场验收清单完成。

# 2026-09-07 技师人体标注服务参考 v5 生产发布

- PR #49 已 squash 合并，主干提交：`4b2764331c86d9dae02452d748d5f08dc1c5b402`。
- GitHub 可信 CI `34116640565`、生产部署 `34116796271` 均成功；监控报告 `deployment_succeeded`。本次无数据库迁移。
- 已发布正反面人体点位四步记录（部位、顾客自述情况、当前状态、本次处理），单次最多三项；背面左右按顾客自身左右保存。
- 权限边界：仅本人完成服务的技师可写；管理端服务端脱敏，仅返回“服务前再确认”摘要；身体明细不进入当前画像、营销、分群或差别定价。
- 待现场验收：使用授权测试账号在微信 390×844 完成点位、四步保存、关闭重开、查看上次、网络失败重试与权限边界；自动部署不等同门店验收。

# 2026-09-06 技师端当前画像数据底座生产发布

- PR #44（画像底座）、PR #45（画像迁移许可）与 PR #46（技师历史归属前置迁移许可）均已 squash 合并；主干发布提交为 `810817dd91d104afa671f063f4919ea92fd4e814`。
- 生产 release：`github-810817dd91d1-34032430331`。GitHub 发布任务 #37 成功：发布前数据库备份校验和恢复演练通过；`20260905_technician_service_history_v3` 与 `20260906_wellness_current` 已按受控顺序升级，Alembic head 为 `20260906_wellness_current`。
- 发布脚本完成 Manifest 校验、原子切换、API 容器更新及 `/`、`/admin/`、`/technician/`、`/api/v1/health` 健康检查。自动化发布不替代门店现场验收。
- 已发布能力仅为按顾客与门店隔离的当前画像投影与管理端受控只读接口；健康自述、顾客原话和技师观察不进入投影。技师真机快速记录、权限与营业闭环仍需使用授权测试账号现场验收。

# 2026-09-05 顾客单设备掉线感知与“我的”刷新修复

- PR #39 已合并，主干 `51db505150c6e598b88c34fc0416afd0f913ebe5`；生产 release 为 `github-51db505150c6-33964908298`，前一版本 `github-ae20a3415cbf-33963739227` 保留。
- 修复旧设备最长等待60秒且后台限频的问题：每5秒校验登录代次，页面重新可见或获得焦点时立即校验；收到 `SESSION_REPLACED` 清除旧登录并提示重新登录。
- 修复刷新“我的”回到项目列表：启动时从受控浏览器历史状态恢复个人中心弹层，不改服务位、选单和价格状态。
- 顾客端 `183 passed, 1 skipped`、生产构建通过；H5 登录专项 `10 passed`。备份 `pre-github-51db505150c6-33964908298-20260905T120358Z.dump` 校验与恢复演练通过，Manifest、原子切换和容器健康检查通过；本次未执行数据库迁移。
- 公网首页、健康接口和新资源 `index-BhzZQLv-.js` / `index-BoAeh19J.css` 均 HTTP 200，健康返回 `status: ok`。待手机与平板现场验收，自动化和公网探针不替代门店验收。

# 2026-09-05 顾客会员本人核验与“我的”升级发布

- 业务 PR #34、迁移发布 PR #35/#36 已合并；生产主干为 `ae20a3415cbf146a27039708f4a77d9c047dcab0`。
- 生产 release：`github-ae20a3415cbf-33963739227`；前一版本 `github-c2c0cebed204-33957196810` 保留。
- 发布前备份 `pre-github-ae20a3415cbf-33963739227-20260905T114331Z.dump` 校验及恢复演练通过；两条加法迁移先在恢复库、再在生产库成功升级，Alembic head 为 `20260905_membership_verification`。
- 发布脚本完成 Manifest、原子切换、API 容器重建和三入口健康检查。公网首页、管理端、技师端、健康接口及实际 JS/CSS 均 HTTP 200，健康返回 `status: ok`。
- 已发布单设备登录、“我的”任务中心、会员有效期与累计节省、待评价/到店详情、唯一可信设备、30秒动态会员码、技师或店长扫码核验、核验后会员价重算及后台设备撤销审计。
- 自动化验证：后端 `598 passed, 7 skipped`；顾客端 `181 passed, 1 skipped`；管理/技师端 `139 passed`；发布专项 `20 passed, 3 skipped`。
- 待现场验收：真实手机/平板摄像头扫码、单设备互踢、设备换绑、弱网/断网以及会员价核验闭环。自动化与公网探针不等同门店营业验收。

# 2026-09-05 自动发布修复与 CI 产物复用

- PR #27 合并 `0dbb1efad621864dd26b6a14e9f12c02e6409eac`，main CI `33951086004` 通过；自动流程 `33951155821` 完成服务器切换，不再由对话手工上传发布。
- 当前 release：`github-0dbb1efad621-33951155821`，旧版 `manual-1f10b35-20260905-4` 保留。镜像 `sha256:a222d36d26fec6282924de2975021f12b06e663fbc341399b7649fd0d68e5e76`，运行正常、重启 0，数据库健康。
- 备份 `pre-github-0dbb1efad621-33951155821-20260905T070721Z.dump`，SHA-256 `42c62fe7f4b56b4f3d7f48d89531a64024f3d1895fd6f9f22a927c59af85984f`，恢复演练通过；Manifest SHA-256 `688956168d8ac93dfb837050826a10db7de5bdc249833a5651986a89898ad73e`，逐文件校验通过。
- 首页、管理端、技师端、健康接口及两张关键长图 HTTP 200；健康 `status: ok`。JS/CSS 保持 `index-BUW-BRvJ.js` / `index-Cu0pxEgk.css`，公网哈希与发布目录一致。未执行数据库迁移，Alembic head 不变。
- 已核对可信主机公钥，经用户授权配置独立受限 SSH 密钥与 production Secrets；不使用个人私钥。凭证预检、任意命令拒绝、受限上传验证通过；凭证不写入仓库或日志。
- 主干 CI 上传按 SHA 绑定的构建产物，发布直接复用；未知/运行时/工作流变化仍全量验证，纯文档保留静态合同、凭证扫描和可信门禁。跨境上传仍可能较慢，不宣称固定耗时或 token 节省比例。
- 新增本地不调用模型的只读观察脚本，后台生成 `output/release-tools/latest-report.json`，避免对话持续轮询。业务 UI 与价格未改，现场验收仍待门店进行。

# 2026-09-05 顾客端二级页价格分组与排版发布

- 业务 PR #25 已合并：`1f10b3591a1eb20857b7684c166e48e4aaa48d66`；与服务器测试源码 `e71c5b8` 的 tree `40feeb60ee46b5529e5170299ef8a701caa60c12` 一致。
- 已发布 `/root/hxy-diy-20260811/releases/manual-1f10b35-20260905-4`；旧版本 `manual-8912faf-20260905-3` 保留。未执行数据库迁移，Alembic 仍为 `20260904_service_reference_v2`。
- API 镜像 `sha256:84361fb787de0ddf9b29ed96dd56ff6a7073aea5356197f83e0670b41a3be9e2`；运行中、重启 0，数据库健康、重启 0。
- 备份 `pre-manual-1f10b35-20260905-4-20260905T060906Z.dump`，SHA-256 `0b67b4e6a361d812c5791bdf7879f0f986c09a6aba5a732d9e2444ec46879bbc`；恢复演练通过。
- Manifest 逐文件通过，SHA-256 `cebde564f166c3e33c563110ab41d549fe3abf5430485280aaa10ea4bcf537a4`。
- 线上入口引用 `index-BUW-BRvJ.js` / `index-Cu0pxEgk.css`，SHA-256 分别为 `b302a614b011c7cc46dbcb71c245cacd0f9a55faf1f8006e28d91f2d2ae64d2c` / `e97219155267bd34ffe0e3575077d88e405af4ebd68aafb113b48cb65131f84c`，与本地及服务器构建一致。
- 首页、管理端、技师端、健康接口、两份新资源、足部精修与女神专享长图均 HTTP 200；健康 `status: ok`，公开领券模板仍为空。
- 本地顾客端 173 通过/1 跳过，后端专项 32 通过；服务器顾客端 173 通过/1 跳过，后端全量 590 通过/4 跳过；共享记忆 5 通过，主干 CI 及 PR 门禁通过。
- 浏览器 48 场景无介绍卡横向溢出，价格标签紧贴金额。保留原 UI、长图、服务介绍和计价规则，未改其他端业务。
- 自动发布 `33948973716` 在固定主机指纹配置失败、上传未执行；受保护手工脚本接续。切换短暂出现一次 502，健康重试恢复，最终探针通过。
- 提速复用了匹配 lockfile 的依赖缓存，完整日志存服务器 `incoming/detail-layout-20260905/`，只返回摘要；未减免备份、恢复演练或校验。服务器先测试因缺少尚未构建的 dist 图片失败，构建后全量重测通过。
- 待现场验收：门店真机检查全部项目介绍、三种身份价格、长图及加购/再次提交；自动化不等同营业现场验收。

# 2026-09-05 顾客端详情介绍精简与暂停发券发布

## 已发布生产

- 用户明确要求“发生产”；业务 PR #23 已合并，主干 `8912faffd02111f00289cc93ec58c10ccdfda37f`，与服务器验证源码 `16e1370` 的 Git tree 一致。
- Release：`/root/hxy-diy-20260811/releases/manual-8912faf-20260905-3`；上一版本 `manual-fd58312185ae-20260905-2` 保留。
- API 镜像：`sha256:35ded3c7399903c6c24e87ffede235764d458e498454bf7d63bfd845c411cc0a`；运行正常、重启 0 次，数据库健康。
- 备份：`pre-manual-8912faf-20260905-3-20260905T051832Z.dump`；SHA-256 `b82b660efba5698545108a9a189acb2695bec34da5ac62178f03a2bed2a45047`，恢复演练通过。
- Manifest 逐文件校验通过，SHA-256 `0485249be920962cc0f55c38fe78c2492d55278117ddb8bc05938ad7de8f19c8`。
- 首页、管理端、技师端及健康接口均 HTTP 200；入口引用 `index-DbL61-n0.js`、`index-D5z4NagC.css`，新文件及足部精修、女神专享长图 HTTP 200。
- 停券开关为 False，公开领券模板数为 0；保留已领券和原有使用规则。
- 本地后端全量 587 通过、7 跳过；服务器后端全量 590 通过、4 跳过；服务器顾客端构建和测试 173 通过、1 跳过。GitHub CI 与 PR 合并门禁通过。
- Alembic 仍为 `20260904_service_reference_v2`，未执行数据库迁移。

## 发布过程与边界

- 自动发布 run `33946809818` 在主机密钥配置步骤失败，部署密钥与上传步骤均未执行；沿用已授权的受保护手工发布流程。
- 服务器使用校验后的 Git 源码归档与仓库历史验证，保留既有管理端构建和静态图片；顾客端重新构建，CSS/JS 与本地哈希一致。使用阿里云 PyPI 与 npm 镜像；服务器系统 Node 未改动，测试使用独立 Node 22。
- API 重建时出现短暂 502，脚本重试后恢复；发布完成后的全部健康与资源探针通过。
- 待现场验证：匿名、非会员、会员的真实项目浏览、偏好选择、加购、金额与再次提交。公网探针及自动测试不等于现场营业验收。
...
## 2026-09-03 CI 门禁稳定性优化

- 修复静态合同空白检查范围过大导致的历史文件末尾空白失败：CI 与 Trusted PR Gate 现仅检查当前提交 diff。
- 更新 GitHub 自动化合同测试；本地 7 项合同测试全部通过。
- 仍未发布生产；推送后等待 PR #10 的新一轮 CI 结果。

## 尚未完成事项

- 任务分支已推送至远端；当前尚无开放 PR，待创建 PR 后运行 CI。
