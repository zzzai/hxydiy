...
# 2026-08-30 技师端源码基线发布就绪性复核（未发布）

## 本地完成

- 在隔离工作树检查候选源码基线 `origin/codex/admin/monorepo-bootstrap` 的提交 `59cafdc`。
- 技师后端专项：`30 passed`；管理端完整测试：`107 passed`；管理端生产构建成功。
- 复核确认技师服务确认/结束幂等键按服务位、动作、登录技师隔离；本轮未新增派单、接单、智慧宝物理资源操作或跨门店读取。

## 发布阻断

- 候选基线全量后端测试：`516 passed, 14 failed, 6 skipped`。发布测试失败的根因是基线缺少 `deploy/` 和 `tmp-codex/publish_body_part_catalogs.py`，不是线上环境问题。
- 其余权限失败来自旧测试夹具创建未绑定的 `staff` 角色；现行认证契约要求拒绝该角色，不能通过降低角色校验修复。

## 发布状态

- 未发布生产；未执行数据库备份、迁移、Manifest 切换或线上健康检查。
- 服务器实际 `current` 未改变，仍以共享记忆记录的 `customer-profile-member-no-coupon-20260829-1` 为准。

## 尚未完成

- 由源码基线负责人补齐经脱敏检查的发布目录和校验脚本，并迁移/澄清旧角色测试夹具后，重新运行全量后端测试。
- 基线全绿并完成 PR 审核后，才创建正式技师发布分支并执行数据库备份、Manifest 校验、健康检查及门店现场闭环。
