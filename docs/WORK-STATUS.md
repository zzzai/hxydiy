# 2026-09-21 门店沙发二维码补齐

- 1 号店沙发展示号为左侧 1、2、3、5，右侧 6、7、8、9；稳定位置编码仍为 `sofa-01` 至 `sofa-08`，避免以后调整展示号导致已印二维码失效。
- 生产写入前已生成并校验备份 `/root/hxy-diy-20260811/backups/pre-sofa-qr-batch-20260921T100928Z.dump`。保留 1 号沙发现有有效码，新增其余 7 个受管二维码；创建审计使用系统身份，未冒用店长或员工账号。
- 服务端已验证 8 个二维码的签名、门店、服务位编码和来源绑定；本地已将 8 张成品图片逐张解码并与生成清单一致性核对，A4 打印版共 4 页。
- 状态为生产数据已配置并完成技术校验，不等于门店现场验收。仍需打印张贴后，用微信真机从每个沙发逐一扫码，确认页面显示的沙发号与现场位置一致。
