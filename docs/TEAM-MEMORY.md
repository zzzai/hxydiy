...
2026-09-05：会员本人使用采用“唯一顾客登录会话＋唯一可信设备＋30秒动态会员码＋风险核验”。动态码由顾客端生成；日常扫码入口在技师端，技师或店长账号按门店和独立 `membership_verify` 权限使用；电脑管理后台仅负责策略、设备换绑审批、异常处置和审计。后端原子完成码消费、本人/门店/选单校验与会员价重算，手机号查询、自由勾选会员或前端改价不得绕过。首期复用现有手机摄像头，不采购专用硬件。本决策当前仅为 PRD，尚未实现或发布。
