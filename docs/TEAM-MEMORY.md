...
2026-09-10：年度会员后台闭环首期确认：退款或取消后，未使用年度赠送权益统一流转为 `voided`；已核销权益保留不可回退的事实记录。若需重开同一周期，只能由店长使用原因、关联周期及幂等键进入受审计异常恢复流程；此流程不得成为手机号直接启用会员价的旁路。开通、续费、赠送服务行核销和本人短信换机的范围与验收见 `docs/product/admin-membership-closure-prd.md`；开发中，尚未合并或发布。
