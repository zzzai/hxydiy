...
2026-09-14：年度赠送服务行核销契约补齐：仅当前门店店长可在已确认、已绑定顾客的选单中，以周期、服务行和幂等键核销一项已发布、门店价不高于 99 元且无加项的未开始服务。服务端原子冻结该行为 0 元、记录审计并在未收款服务账单同步；取消该服务行仅在周期仍 `active` 时释放权益，已退款/取消而作废的权益不得重新开放。详见 `contracts/admin-membership-closure.md`；本地开发中，尚未合并或发布。
