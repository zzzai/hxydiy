...
2026-09-10：会员核验是会员权益的按需步骤，普通顾客不进入核验。扫码先识别会员、再选择本次选单进行原子绑定；技师看板和候选接口只认仍持有活动服务位关联的记录，服务结束后为下一单保留的历史不应构成冲突。真实存在多条活动关联时，候选接口才排除该服务位并返回最小待核对提示。详见 `contracts/customer-membership-verification.md`。
