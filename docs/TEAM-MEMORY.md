...
| 2026-09-09 | 管理端媒体上传和列表只返回稳定 `/api/v1/admin/media/{id}/content` 引用；对象存储的短期签名 URL 仅在受门店权限控制的内容读取请求中生成，目录不得保存会过期的 CDN 地址。上传只接受允许的图片 MIME 与对应扩展名，且限制 5MB。删除先检查同店已发布项目、加项、商品及项目详情模块的稳定引用；命中时返回 `409 MEDIA_IN_USE`，未命中时仅软删除元数据，物理对象清理另行设计。详见 `contracts/media-delivery-v1.md`。 | 管理端、后端 |
