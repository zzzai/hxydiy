...
2026-09-10：会员核验是会员权益的按需步骤，普通顾客不进入核验。扫码先识别会员、再选择本次选单进行原子绑定；同一服务位有多条活动占用时，候选接口排除该服务位并只返回待核对的最小冲突提示，不能将会员绑定到歧义选单。详见 `contracts/customer-membership-verification.md`。
