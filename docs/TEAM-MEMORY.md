...
2026-09-08：门店运营汇总的 CSV 导出只包含固定聚合指标，不含顾客姓名、手机号、openid、订单明细或服务参考。绑定门店的管理员只能导出本店；未绑定门店的总部管理员必须显式指定一个存在的门店，省略范围不得退化为全门店导出。每次导出写入 `export_operations_summary` 审计。首店阶段同步返回 CSV；异步任务、临时文件下载和跨店汇总留待百店规模再设计。
