...
2026-09-14：管理端媒体删除增加同店引用保护：项目主图/详情模块、加项、商品与页面内容仍引用媒体时，服务端返回 `409 MEDIA_IN_USE`，不删除对象或软删除元数据；检查同时覆盖受控媒体路径与历史签名 CDN 地址中的对象键。解除引用后才可删除，既有门店隔离、权限与审计边界不变。详见 `contracts/admin-media.md`。
