...
| G5 | 会员权益跨店规则：membership_store_scope 已按店隔离，品牌级通用还是门店限定 | 业务 | 已决策：品牌通用（见 8） |
...
## 8. 业务决策（2026-09-22 用户确认）

1. **G5 会员权益跨店规则 → 品牌通用**。会员身份与权益跨店共享；membership 域不新增品牌级作用域列，membership_store_scope 保留用于按店统计/运营视图，不构成权益边界。
2. **门店 2 物理参数 → 未选址，挂起**。G4 门店 2 供给迁移暂缓：schema 先行交付，数据迁移（stores/rooms/床位种子）待选址确定、物理参数齐备后单独立项；PR4 仅交付可参数化的迁移骨架与常量位，不写入任何门店 2 实际数据。
3. **顾客端门店选择 → 二维码分流**。不建店选页、不设独立站点；顾客入口继续以房间/沙发二维码（rooms.code，G1 编码规范）为唯一分流通道，diy.hexiaoyue.com 的 store_id=1 硬绑定随 G3 改造改为"按 QR 上下文解析门店"。
4. **会员价开关 → 随 PR1 立项**。member_price_enabled（catalog 退役 backlog）纳入 PR1 范围，与目录模板 schema 同批交付；语义按候选 backlog 记录：门店级开关，控制 member_price_cents 是否对顾客生效，默认关闭，仅总部/店长可切换，切换入审计。
...
- [x] 业务决策已由用户逐项确认（2026-09-22，见第 8 节）
