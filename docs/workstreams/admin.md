...
- 后端合同测试覆盖创建后可登录、密码不回显、审计范围、列表隔离、调岗撤销旧会话、总部/技师排除和店长禁止维护；前端模型与导航边界测试通过。此本机未安装前端依赖（`tsc` 不可用），完整前端构建留给 PR CI。契约见 `../contracts/admin-staff-accounts.md`。
