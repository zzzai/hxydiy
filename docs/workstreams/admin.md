...
## 2026-09-14 已支付项目销量（本地完成，未合并）

- 经营分析新增按所选日期、当前门店聚合的已支付项目销量 Top5，保留订单冻结名称、数量和去重订单数；不将其作为财务结算或分摊优惠金额。
- 汇总只读取本店未取消、未退款的已支付订单，不返回顾客信息；项目浏览热度保留为独立指标。
- 后端合同测试 44 passed；管理端页面测试 2 passed；`git diff --check` 通过。本机未安装锁定的管理端依赖，`npm run build` 在 `tsc` 缺失处停止，构建验证待 CI 执行。

## 2026-09-14 媒体删除引用保护（已合并，未发布）
...
- 本地后端媒体与存储回归 13 passed（1 个既有 TestClient 弃用警告），`git diff --check` 通过；PR #85 已于 2026-09-14 合并至 `main`（`20fafaa`），可信 CI `34803424268` 成功。生产部署工作流 `34803530982` 已取消，服务器 `current` 未切换；现场验收应覆盖门店隔离、引用保护、解除引用后的删除和审计。

## 2026-09-14 年度会员闭环（已合并，未发布）
...
- 已实现并完成本地合同测试：年度赠送服务行核销、幂等重试、服务行取消释放和未收款账单同步，以及本人短信换机撤销旧入口；契约见 `../contracts/admin-membership-closure.md`。PR #82 已于 2026-09-14 合并至 `main`（`539b473`），后端、管理端、顾客端构建与静态契约检查均通过；未发布生产。
...
- 后端合同测试覆盖创建后可登录、密码不回显、审计范围、列表隔离、调岗撤销旧会话、总部/技师排除和店长禁止维护；PR #89 已于 2026-09-14 合并至 `main`（`efb8c18`），可信 CI 的后端、管理端构建、顾客端构建和静态契约检查均成功。服务器 `current` 未切换，未发布生产。契约见 `../contracts/admin-staff-accounts.md`。
