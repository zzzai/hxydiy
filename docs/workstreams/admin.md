...
## 2026-09-10 年度会员闭环（开发中，未发布）

- 已确认退款/取消规则：未使用年度赠送权益统一作废；已核销权益保持事实不可回退；仅允许店长以原因、关联周期和幂等键走受审计异常恢复。
- 首期覆盖管理端开通、到期前 90 天续费、取消/退款、一次赠送服务行核销、换机申请审核与异常恢复；不新增后台按手机号直接套会员价入口。
- 日常会员本人核验仍由顾客动态码和技师端扫码完成；管理后台只负责受控换绑、审计及业务管理。
- 需求细节与验收约束：`docs/product/admin-membership-closure-prd.md`。
- 已实现并完成本地合同测试：年度赠送服务行核销、幂等重试、服务行取消释放和未收款账单同步；契约见 `../contracts/admin-membership-closure.md`。换机申请/审核仍需先与主干“短信自助换绑”规则统一，未合并或发布。
