...
## 2026-09-08 门店运营汇总 CSV 导出（本地完成，未发布）

- 经营分析页可按当前日期范围下载固定聚合指标 CSV，复用既有运营汇总口径；不导出顾客姓名、手机号、openid、订单明细或服务参考。
- 绑定门店的管理员只能导出本店；未绑定门店的总部管理员必须显式指定一个门店，不能因遗漏参数导出全门店。每次下载写入 `export_operations_summary` 审计，审计仅记录门店、日期范围和格式。
- 首店阶段同步返回下载内容，不新增任务队列、文件留存或跨店汇总；这些能力在百店规模迁移时统一设计。

验证：后端 API 合同测试 `46 passed`；管理端完整测试 `168 passed`；`npm run build` 通过。均含既有 Starlette/httpx 弃用警告，未执行浏览器或门店现场验收。

发布状态：仅本地完成，未发布生产；未执行数据库迁移、备份、Manifest、服务器切换或线上健康检查，故不更新 `CURRENT-STATE.md` 和 `WORK-STATUS.md`。
...
- 状态：管理端 v3 门店范围只读摘要已在本地完成；基础聚合仍待后续独立开发
- 负责人：管理端窗口
