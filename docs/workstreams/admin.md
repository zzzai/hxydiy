...
- 现有扫码进入响应签发 `visit_feedback_token`，不要求重印二维码；提交使用数据库幂等键、共享数据库限流、严格标签/文本校验和独立埋点。
- 管理端“顾客反馈”统一列出到店反馈与服务评价，明确类型和来源，支持真实详情、处理状态、备注与审计；所有读取和处理按门店隔离。
- 当前仍处于本地开发，未合并、未发布生产；数据库迁移、恢复演练、CI、独立审查和生产验收尚未完成。

## 2026-09-21 连锁品牌账号授权底座（开发中）
...
- 当前仅本地开发，尚未推送 PR、合并、迁移生产或现场验收；不包含项目模板、价格分层或 React 工作区界面。
