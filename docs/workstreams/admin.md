...
- PR #130 已合并，随主干 `34fab31` 上线；`staff_scope_assignments` 已迁移。生产 `admin` 的 1 号店店长授权已创建但因管理前端尚未提供多工作区选择页而临时停用，品牌管理员工作区继续可用；修复选择流程并验证后再恢复该授权。现场权限和二维码打印验收尚未执行，不包含项目模板或价格分层。
