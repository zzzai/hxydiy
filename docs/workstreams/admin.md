...
## 2026-09-24 门店 1「辅」目录调整（开发中）

- 目标六项及定价见 `../contracts/auxiliary-menu-v1.md`。生产旧拔罐 ID 8、刮痧 ID 9 仍被已发布主项目目录引用，不能停用或改写；新增 `independently_visible` 仅控制顾客独立项目列表，默认 true，后台和历史详情继续可读。
- 门店 1 数据调整由 `hxy-server/scripts/reconcile_aux_menu.py` 先 dry-run 再经备份/迁移后显式 `--apply`；旧价格和历史快照保留。新增精油开背与必选单选的拔罐/刮痧，足部精修门店价以新价格版本改为 69 元。
- 当前为隔离分支本地开发，尚未合并或发布。顾客端硬编码的“腰臀”需由顾客端窗口同步处理；本窗口不交叉修改顾客端组件。现场扫码、选单与门店验收交人执行。

三端统一交付：按 `../script-first-delivery.md` 使用共享后台脚本等待，下一次只读最终报告，失败才介入；本条不改变管理端业务范围。

更新时间：2026-09-21

## 2026-09-21 到店反馈与管理端处理闭环（已发布，待现场验收）

- 保留已完成服务评价的本人所有权、权威结束和一单一评，新增独立 `visit_feedback` 持久化；未关联反馈不创建假服务单，不从服务位猜订单、顾客或技师。
- 每个实际服务位使用独立签名二维码；只有验证签名的入口可取得 `visit_feedback_token`。提交使用数据库幂等键、共享数据库限流、严格标签/文本校验和独立埋点。
- 管理端“顾客反馈”统一列出到店反馈与服务评价，明确类型和来源，支持真实详情、处理状态、备注与审计；所有读取和处理按门店隔离。
- PR #133 已合并，随主干 `34fab31` 和生产部署 `35834287369` 上线；`visit_feedback` 表已迁移。门店扫码与处理闭环仍待现场验收。

## 2026-09-21 连锁品牌账号授权底座（已发布，待现场验收）

- 新增品牌管理员、总部运营、店长、门店员工的多角色/多范围授权，以及登录工作区发现、显式选择和作用域令牌；技师登录保持原契约。
- 新增总部授权管理接口、最后一个品牌管理员保护和完整授权审计上下文；账号、凭证或授权撤销会使相关令牌失效。
- 历史账号迁移保留密码且不扩大权限；受保护的 `admin` 升级为品牌管理员、解除固定门店并撤销旧会话。
- PR #130 已合并，随主干 `34fab31` 上线；`staff_scope_assignments` 已迁移。PR #136 补齐多工作区选择页并以主干 `ff3f467` 发布；生产 `admin` 的 1 号店店长授权 ID 4 已恢复，品牌管理员授权继续有效，旧会话因凭证版本递增而失效。真实账号的双工作区进入及二维码管理页面仍待人工验收，不包含项目模板或价格分层。

## 2026-09-21 商品目录首期可运营闭环（已发布，待现场验收）

- 总部商品表单新增可选会员价、展示顺序和文字详情；列表同步展示会员价与顺序，店长权限仍仅限本店上下架。
- 后端新增对应字段和迁移；会员价不得高于门店价，详情模块只接受受控文字或图片类型，公开目录按展示顺序稳定输出。
- 不新增库存、采购、在线支付、自提、配送或顾客端商品下单；PR #121 已通过 14 项检查并合并为 `7edae827787df5ff0d25350b6d08e9e2e6a70615`。
- 本地验证：管理端 200 项测试、TypeScript 与生产构建通过；后端完整套件 686 passed、7 skipped、70 个子测试通过，仅有既有 TestClient 弃用警告；项目静态合同 34 passed；OpenAPI 和生成类型二次生成哈希一致。
- 生产证据：迁移许可 PR #123 合并为 `c76ac301efa07f803d251453a5c99151b4a62dc4`；CI `35575952327`、生产部署 `35576431361`、备份恢复演练、`20260921_product_catalog` 迁移、release 切换及公网探针全部通过。状态为 `Deployed`，总部/店长授权写入与门店营业现场仍待验收。
- 增量规格：`../specs/ADMIN-CATALOG-002.md`；契约：`../contracts/admin-catalog-openapi-v1.md`。

## 2026-09-21 项目/商品 OpenAPI 类型收敛（已合并，工具链无需单独发布）

- 项目与商品主要管理接口补齐机器可读响应模型和 Bearer 鉴权说明，保留旧数组/分页双响应与 POST 更新兼容，不改变业务行为。
- 管理端项目、商品模型改为引用 OpenAPI 生成类型；固化规格和类型均由命令生成，CI 与可信 PR 门禁检查漂移。
- Schemathesis 首期只在隔离测试库对项目、商品 GET 列表生成查询并校验响应；写接口和生产环境均不执行随机请求。
- 契约见 `../contracts/admin-catalog-openapi-v1.md`。PR #118、#119、#120 已合并至主干；该治理工具链不等同于业务生产发布或现场验收。

## 2026-09-16 七牛直传与断点续传（已合并，未发布）

- 管理端媒体上传已接入七牛 JavaScript SDK 的默认分片与断点续传；本期不额外设定分片阈值或并发策略。服务端限时签发绑定员工、门店、文件元数据和暂存对象键的上传凭证；完成回调复读暂存对象，校验真实图片内容和像素后才转为正式对象、写入媒体记录和审计。
- 暂存对象仅允许一次写入且受大小、MIME、用途及门店范围限制；重复完成返回同一媒体记录。七牛未配置时，管理端回退既有服务端上传路径。病毒扫描按产品决定明确留待后续独立闭环，不能将现有图片校验称为扫描。
- PR #103 已合并；PR #104 仅修复 Windows CRLF 导致的静态检查失败，随后主干 `1e2898b` 的后端、管理端、顾客端、静态契约与可信 PR 门禁均成功。生产部署工作流未触发，Bucket CORS 与暂存路径生命周期仍是启用直传前置条件，现场验收未完成。

## 2026-09-14 已支付项目销量（已发布）

- 经营分析新增按所选日期、当前门店聚合的已支付项目销量 Top5，保留订单冻结名称、数量和去重订单数；不将其作为财务结算或分摊优惠金额。
- 汇总只读取本店未取消、未退款的已支付订单，不返回顾客信息；项目浏览热度保留为独立指标。
- 后端合同测试 44 passed；管理端页面测试 2 passed；`git diff --check` 通过。PR #87 已合并，且已随 `efb8c18` 于发布任务 `34810436752` 部署生产；现场验收未完成。

## 2026-09-14 媒体删除引用保护（已发布）

- 删除媒体前检查同店项目主图/详情模块、加项、商品与页面内容配置；仍被引用时返回 `409 MEDIA_IN_USE`，不删除对象或软删除元数据。
- 引用识别同时覆盖受控媒体路径与历史签名 CDN 地址中的对象键，避免旧内容因误删图片失效；解除引用后仍沿用存储删除成功才软删除元数据的顺序。
- 本地后端媒体与存储回归 13 passed（1 个既有 TestClient 弃用警告），`git diff --check` 通过；PR #85 已于 2026-09-14 合并至 `main`（`20fafaa`），可信 CI `34803424268` 成功。此前自动触发的发布 `34803530982` 已取消；后续维护者手动发布 `34810436752` 成功，服务器 `current` 已切换至包含该变更的 `efb8c18`。现场验收应覆盖门店隔离、引用保护、解除引用后的删除和审计。

## 2026-09-14 年度会员闭环（已发布）

- 已确认退款/取消规则：未使用年度赠送权益统一作废；已核销权益保持事实不可回退；仅允许店长以原因、关联周期和幂等键走受审计异常恢复。
- 首期覆盖管理端开通、到期前 90 天续费、取消/退款、一次赠送服务行核销、本人短信自助换机与异常恢复；不新增后台按手机号直接套会员价入口。
- 日常会员本人核验仍由顾客动态码和技师端扫码完成；本人短信换机由服务端撤销旧入口并审计，管理后台不参与审批。
- 需求细节与验收约束：`docs/product/admin-membership-closure-prd.md`。
- 已实现并完成本地合同测试：年度赠送服务行核销、幂等重试、服务行取消释放和未收款账单同步，以及本人短信换机撤销旧入口；契约见 `../contracts/admin-membership-closure.md`。PR #82 已于 2026-09-14 合并至 `main`（`539b473`），后端、管理端、顾客端构建与静态契约检查均通过；已随 `efb8c18` 于发布任务 `34810436752` 部署生产，现场验收未完成。

## 2026-09-09 服务位维修备注与展示顺序（本地完成，未发布）

- 新增严格的服务位配置接口，复用既有 `Room.note` 与 `Room.sort_order`；不新增数据库列或 Alembic 迁移。
- 本期只支持绑定门店店长维护本店实际服务位；普通员工继续只读且服务位地图不接收维修备注，总部跨店选店不在本期范围。
- 维修备注与展示顺序不会改变服务位启停、占用、服务单、二维码或智慧宝物理资源。完整契约见 `../contracts/service-position-configuration-v1.md`。
- 验证：后端服务位/权限回归 57 passed（1 个既有 TestClient 弃用警告）；管理端完整测试 177 passed；`npx tsc -b --pretty false` 退出码 0；`git diff --check` 通过。未执行浏览器、PR、生产发布或门店现场验收。

## 2026-09-08 正式门店员工只读角色（本地完成，未发布）

- 用户确认此前创建的账号均为测试账号；只有状态为 `active`、绑定门店的在职员工可作为正式 `staff` 登录。
- 普通员工仅显示“今日运营、服务位看板、到店服务选单”。今日页只展示本店当日预约，不显示金额、经营统计或状态变更入口。
- 后端以请求级只读白名单保护普通员工，仅允许读取当日预约、服务位现场图、既有服务位二维码和已提交选单；其余管理接口（包括评价、确认/取消选单、结算、目录与资源配置）统一拒绝为 `403 STAFF_READ_ONLY`，不依赖前端菜单隐藏。
- 选单只读接口只返回 `id`、入口来源、设备名称、状态、项目名称/数量和提交时间；不返回顾客、会员、评价、偏好、价格或计价快照。该字段投影由后端回归测试覆盖。

## 2026-09-14 门店员工账号管理（已发布）

- 总部管理员新增“员工账号”页面，可分页筛选并创建、编辑正式门店 `manager` / `staff` 账号；创建需要初始密码，编辑时密码留空不会提交，登录名创建后不可修改。
- 门店、角色、状态或密码变化会立即撤销旧会话；创建、调岗与更新审计按目标门店范围落库，且不保存或回显密码。总部账号和技师账号不通过该入口维护。
- 后端合同测试覆盖创建后可登录、密码不回显、审计范围、列表隔离、调岗撤销旧会话、总部/技师排除和店长禁止维护；PR #89 已于 2026-09-14 合并至 `main`（`efb8c18`），可信 CI 的后端、管理端构建、顾客端构建和静态契约检查均成功。维护者手动发布 `34810436752` 成功，服务器 `current` 已切换；现场验收未完成。契约见 `../contracts/admin-staff-accounts.md`。

验证：按锁文件执行 `npm ci --ignore-scripts` 后，管理端完整测试 `171 passed`、`npx tsc -b` 通过；后端权限与服务位专项 `53 passed`（含 1 个既有 Starlette/httpx 弃用警告），`python -m compileall -q app` 与 `git diff --check` 通过。生产构建已启动但本机执行窗口在 Vite 转换阶段超时，需由 CI 复验；未执行浏览器或生产操作。

发布状态：代码已提交、推送并创建 PR；尚未合并或发布生产，未执行数据库迁移、备份或服务器切换。

## 2026-09-05 服务参考 v3 只读展示（本地完成，未发布）

- 到店服务选单内按 `schema_version=3`、`taxonomy_version=service_reference_v2` 展示六组结构化摘要，并显示来源、记录时间、确认状态和确认时间；相关情况原话默认折叠。
- 页面只消费现有门店隔离历史接口，移除该位置新增/更正画像入口；不写普通标签、搜索索引或算法特征。旧 v1/v2 保持只读兼容，不自动升级。
- 当前管理页使用内置白名单中文展示映射，并已与现有 taxonomy 文案对齐；尚未消费技师专用字典接口。后续管理角色字典接口可用后再改为消费服务端权威字典，不伪造或越权复用现有接口。
- 稳定码、来源、确认、可见范围、安全摘要白名单、消费偏好限制及兼容规则见 `../contracts/service-reference-v2.md`。
- 当前仅本地完成，未发布生产；备份/迁移、Manifest、服务器切换、健康检查、权限穿透、移动端及门店验收待实际发布窗口补证，故不更新 `CURRENT-STATE.md` 或 `WORK-STATUS.md`。

## 当前任务

- 状态：管理端 v3 门店范围只读摘要和服务参考基础聚合已完成本地开发，尚待 PR 可信检查、合并、生产发布与门店验收
- 负责人：管理端窗口
- 开始前读取：`../CONTEXT-MANIFEST.md`、`../CURRENT-STATE.md` 和本文件

## 服务参考待办

- 管理页面已按 `schema_version=3` / `service_reference_v2` 只读展示顾客表达、技师观察、顾客确认和下次建议，同时兼容旧记录。
- 标签字典由后端作为唯一标准源，管理端只消费编码、中文展示、版本和启停状态。
- 第一阶段只增加门店范围的基础数量、确认率和更正率，不建设复杂标签配置中心或实时数据仓库。
