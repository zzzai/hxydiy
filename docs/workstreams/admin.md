...
## 2026-09-09 经营报表项目销量与技师服务量（本地完成，未发布）

- 经营分析按当前日期范围增加“项目销量 Top5”和“技师服务量 Top5”，只统计本店实际已结束的服务位；顾客结构同时显示同口径的会员和非会员人数。
- 项目名称和数量来自已保存的选单快照；技师仅按明确服务归属统计，未归属或缺失档案的历史记录不猜测分配。
- 进行中或未释放的服务位不计入周转次数，并保持报表可读取。
- 不展示顾客、会员、价格、提成或服务参考；不增加导出、结算、迁移或智慧宝物理资源操作。完整字段与权限边界见 `../contracts/operations-reporting-v1.md`。

## 2026-09-09 媒体稳定引用（本地完成，未发布）

- 新上传媒体统一返回并保存受控内容地址；对象存储短期签名 URL 只在读取请求中临时生成，不再写入目录表单。
- 项目详情模块可通过上移、下移调整展示顺序，直接复用已保存的数组顺序；本期不引入拖拽依赖或新增排序字段。
- 上传限制为允许图片 MIME 的对应扩展名、服务端解析的实际格式、5MB 和默认 2,000 万像素；审计记录解析后的宽高，避免仅凭前端文件名写入错误对象类型。
- 媒体对象键按门店和白名单用途分层，未知用途请求返回 `422`，不允许用途字段参与任意路径拼接。
- 删除前检查同店已发布项目、加项、商品及项目详情模块的稳定引用；仍被引用时返回 `409 MEDIA_IN_USE`，未命中时只软删除元数据，不在请求中物理删除对象。
- 本期不迁移历史外部 URL，也不扩展裁剪、压缩、病毒扫描、排序、对象清理保留期或恢复入口。完整边界见 `../contracts/media-delivery-v1.md`。
