...
- 技师今日服务接口不再将未归属房间的 `bed` 记录作为独立服务位返回。
- 保持“沙发 + 房间”两类区位展示；房间仍按一个独立单位展示，不拆分床位。
- 未加入派单、接单、认领或智慧宝物理资源操作。
...
- 新增孤立床位回归测试；技师后端相关筛选专项 `38 passed, 1 warning`。
- 管理端测试 `107 passed`；管理端生产构建成功（保留既有共享 chunk 体积警告）。
- 本地完成，未发布生产；服务器 `current` 未改变，未执行数据库迁移或生产切换。

### 待门店现场验收

- 仍需使用授权手机验证顾客提交后区位状态、选单查看、确认服务、服务结束、画像快记保存/失败重试和审计闭环。
- 自动化测试不替代真实手机、断网、并发及智慧宝并行操作验收。
